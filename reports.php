<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Reports &amp; Analytics — TrackMySpend</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,400;0,9..144,500;0,9..144,600;0,9..144,700;1,9..144,500&family=IBM+Plex+Mono:wght@400;500;600&family=IBM+Plex+Sans:wght@400;500;600&display=swap" rel="stylesheet">
<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.4/chart.umd.min.js"></script>
<style>
  :root{
    --cover: #101c16;
    --cover-deep: #0a1410;
    --paper: #f4efe1;
    --paper-dim: #ece5d2;
    --ink: #1c2620;
    --ink-soft: #4b5850;
    --money: #3f7a5c;
    --money-deep: #2c5842;
    --stamp: #b1462f;
    --gold: #c9a15a;
    --line: rgba(28,38,32,0.14);
    --line-on-cover: rgba(244,239,225,0.16);
  }

  *{ margin:0; padding:0; box-sizing:border-box; }
  html,body{ min-height:100%; }
  body{
    background:var(--paper);
    color:var(--ink);
    font-family:'IBM Plex Sans', sans-serif;
    -webkit-font-smoothing:antialiased;
  }
  @media (prefers-reduced-motion: reduce){
    *{ animation-duration:0.001ms !important; animation-iteration-count:1 !important; transition-duration:0.001ms !important; }
  }
  a{ color:inherit; text-decoration:none; }
  .display{ font-family:'Fraunces', serif; }
  .mono{ font-family:'IBM Plex Mono', monospace; }
  ::selection{ background:var(--gold); color:var(--cover-deep); }

  .wrap{ max-width:1180px; margin:0 auto; padding:0 32px; }

  /* ---------------- NAV ---------------- */
  nav{ background:var(--cover); border-bottom:1px solid var(--line-on-cover); }
  .nav-inner{
    display:flex; align-items:center; justify-content:space-between;
    padding:16px 32px; max-width:1180px; margin:0 auto;
  }
  .brand{
    display:flex; align-items:center; gap:11px;
    font-family:'Fraunces', serif; font-weight:600; font-size:1.1rem; color:var(--paper);
  }
  .brand .mark{
    width:32px; height:32px; border-radius:7px; overflow:hidden; flex:none;
    background:var(--paper); box-shadow:0 2px 0 var(--money-deep);
  }
  .brand .mark img{ width:100%; height:100%; object-fit:cover; display:block; }
  .nav-links{ display:flex; gap:20px; font-size:0.86rem; }
  .nav-links a{ position:relative; color:rgba(244,239,225,0.68); transition:color 0.2s; padding-bottom:4px; }
  .nav-links a::after{ content:''; position:absolute; left:0; right:0; bottom:0; height:2px; background:var(--gold); transform:scaleX(0); transform-origin:left; transition:transform 0.25s ease; }
  .nav-links a:hover{ color:var(--paper); }
  .nav-links a:hover::after{ transform:scaleX(1); }
  .nav-links a.active{ color:var(--paper); }
  .nav-links a.active::after{ transform:scaleX(1); }
  .nav-user{ display:flex; align-items:center; gap:10px; }
  .nav-avatar{
    width:32px; height:32px; border-radius:50%;
    background:var(--money); color:var(--paper);
    display:flex; align-items:center; justify-content:center;
    font-family:'IBM Plex Mono', monospace; font-weight:600; font-size:0.78rem;
    border:1.5px solid var(--gold);
  }
  .nav-signout{
    font-size:0.82rem; color:rgba(244,239,225,0.55); border:1px solid var(--line-on-cover);
    padding:7px 13px; border-radius:5px; transition:opacity 0.2s;
  }
  .nav-signout:hover{ opacity:0.7; }
  @media (max-width: 760px){ .nav-links{ display:none; } }

  /* ---------------- PAGE HEADER ---------------- */
  .page-head{ padding:44px 0 0; }
  .kicker{
    font-family:'IBM Plex Mono', monospace; font-size:0.72rem; letter-spacing:0.1em; text-transform:uppercase;
    color:var(--money-deep); margin-bottom:8px;
  }
  .page-head h1{ font-family:'Fraunces', serif; font-weight:600; font-size:2rem; }
  .page-head p{ margin-top:8px; color:var(--ink-soft); font-size:0.96rem; }

  /* ---------------- FILTER BAR ---------------- */
  .filter-bar{
    margin-top:26px; background:#fff; border:1px solid var(--line); border-radius:10px;
    padding:18px 22px; display:flex; align-items:center; justify-content:space-between; gap:16px; flex-wrap:wrap;
  }
  .filter-group{ display:flex; align-items:center; gap:16px; flex-wrap:wrap; }
  .seg{
    display:flex; background:var(--paper-dim); border-radius:7px; padding:3px; gap:2px;
  }
  .seg button{
    border:none; background:none; padding:8px 16px; border-radius:5px; font-size:0.82rem; font-weight:600;
    color:var(--ink-soft); cursor:pointer; font-family:'IBM Plex Sans', sans-serif; transition:background 0.15s, color 0.15s;
  }
  .seg button.active{ background:var(--cover); color:var(--paper); }
  .field-select{
    display:flex; align-items:center; gap:8px;
    border:1px solid var(--line); border-radius:7px; padding:8px 12px;
    font-family:'IBM Plex Mono', monospace; font-size:0.8rem; color:var(--ink-soft);
  }
  .field-select select{
    border:none; background:none; font-family:'IBM Plex Mono', monospace; font-size:0.82rem;
    color:var(--ink); font-weight:600; outline:none; cursor:pointer;
  }
  .export-group{ display:flex; gap:10px; }
  .btn{
    padding:10px 18px; border-radius:6px; font-weight:600; font-size:0.84rem; border:none; cursor:pointer;
    transition:transform 0.15s ease, box-shadow 0.15s ease; font-family:'IBM Plex Sans', sans-serif;
    display:inline-flex; align-items:center; gap:7px;
  }
  .btn-primary{ background:var(--money-deep); color:var(--paper); box-shadow:0 3px 0 #1e3f2f; }
  .btn-primary:hover{ transform:translateY(-1px); box-shadow:0 4px 0 #1e3f2f; }
  .btn-primary:active{ transform:translateY(1px); box-shadow:0 1px 0 #1e3f2f; }
  .btn-ghost{ background:none; border:1px solid var(--line); color:var(--ink-soft); }
  .btn-ghost:hover{ background:var(--paper-dim); }

  /* ---------------- SUMMARY CARDS ---------------- */
  .stats-row{
    margin-top:22px; display:grid; grid-template-columns:repeat(4,1fr); gap:1px;
    background:var(--line); border:1px solid var(--line); border-radius:10px; overflow:hidden;
  }
  .stat{ background:#fff; padding:22px 24px; }
  .stat .label{ font-family:'IBM Plex Mono', monospace; font-size:0.66rem; letter-spacing:0.06em; text-transform:uppercase; color:var(--ink-soft); }
  .stat .value{ font-family:'Fraunces', serif; font-weight:600; font-size:1.55rem; margin-top:6px; color:var(--ink); }
  .stat .sub{ font-size:0.78rem; color:var(--ink-soft); margin-top:4px; }
  .stat .sub.up{ color:var(--money-deep); }
  .stat .sub.down{ color:var(--stamp); }

  /* ---------------- CHART GRID ---------------- */
  .chart-grid{ margin-top:24px; display:grid; grid-template-columns: 1fr 1fr; gap:24px; align-items:start; }
  .panel{ background:#fff; border:1px solid var(--line); border-radius:10px; padding:26px 28px; }
  .panel + .panel{ margin-top:24px; }
  .panel-head{ display:flex; align-items:flex-start; justify-content:space-between; margin-bottom:20px; gap:12px; }
  .panel-head h3{ font-family:'Fraunces', serif; font-weight:600; font-size:1.14rem; }
  .panel-head .hint{ font-family:'IBM Plex Mono', monospace; font-size:0.68rem; color:var(--ink-soft); white-space:nowrap; }
  .chart-box{ position:relative; height:250px; }
  .legend-row{ display:flex; flex-wrap:wrap; gap:10px 18px; margin-top:16px; padding-top:16px; border-top:1px dashed var(--line); }
  .legend-item{ display:flex; align-items:center; gap:7px; font-size:0.8rem; color:var(--ink-soft); }
  .legend-dot{ width:9px; height:9px; border-radius:50%; flex:none; }

  /* ---------------- BREAKDOWN TABLE ---------------- */
  .table-wrap{ margin-top:24px; }
  table{ width:100%; border-collapse:collapse; }
  thead th{
    text-align:left; font-family:'IBM Plex Mono', monospace; font-size:0.66rem; letter-spacing:0.06em;
    text-transform:uppercase; color:var(--ink-soft); padding:0 0 12px; border-bottom:1px solid var(--line);
  }
  thead th.num, tbody td.num{ text-align:right; }
  tbody td{ padding:13px 0; border-bottom:1px solid var(--line); font-size:0.88rem; }
  tbody tr:last-child td{ border-bottom:none; }
  .cat-name{ display:flex; align-items:center; gap:10px; font-weight:600; }
  .cat-dot{ width:10px; height:10px; border-radius:50%; flex:none; }
  td.num{ font-family:'IBM Plex Mono', monospace; }
  .bar-cell{ width:160px; }
  .mini-bar{ height:6px; border-radius:4px; background:var(--paper-dim); overflow:hidden; }
  .mini-bar .fill{ height:100%; border-radius:4px; background:var(--money); }

  /* ---------------- DOWNLOAD CARDS ---------------- */
  .download-grid{
    margin:24px 0 80px; display:grid; grid-template-columns:1fr 1fr; gap:24px;
  }
  .download-card{
    background:var(--cover); color:var(--paper); border-radius:12px; padding:28px 30px;
    position:relative; overflow:hidden; display:flex; flex-direction:column; gap:14px;
  }
  .download-card::before{
    content:''; position:absolute; inset:0;
    background:
      radial-gradient(ellipse 70% 60% at 90% 0%, rgba(201,161,90,0.14), transparent 60%),
      radial-gradient(ellipse 60% 60% at 0% 100%, rgba(63,122,92,0.18), transparent 60%);
    pointer-events:none;
  }
  .download-card > *{ position:relative; z-index:1; }
  .download-card .d-kicker{
    font-family:'IBM Plex Mono', monospace; font-size:0.68rem; letter-spacing:0.08em; text-transform:uppercase; color:var(--gold);
  }
  .download-card h3{ font-family:'Fraunces', serif; font-weight:600; font-size:1.3rem; }
  .download-card p{ font-size:0.86rem; color:rgba(244,239,225,0.68); line-height:1.5; }
  .download-actions{ display:flex; gap:10px; margin-top:6px; flex-wrap:wrap; }
  .dl-btn{
    padding:10px 18px; border-radius:6px; font-size:0.82rem; font-weight:600; cursor:pointer;
    border:1px solid var(--line-on-cover); color:var(--paper); background:rgba(244,239,225,0.06);
    display:inline-flex; align-items:center; gap:7px; transition:background 0.2s;
    font-family:'IBM Plex Sans', sans-serif;
  }
  .dl-btn:hover{ background:rgba(244,239,225,0.14); }
  .dl-btn.gold{ background:var(--gold); color:var(--cover-deep); border-color:var(--gold); }
  .dl-btn.gold:hover{ background:#d9b46e; }
  .dl-note{
    font-family:'IBM Plex Mono', monospace; font-size:0.7rem; color:var(--money-deep);
    opacity:0; transition:opacity 0.3s ease;
  }
  .dl-note.show{ opacity:1; }

  @media (max-width: 900px){
    .chart-grid, .download-grid{ grid-template-columns:1fr; }
    .stats-row{ grid-template-columns:1fr 1fr; }
    .filter-bar{ flex-direction:column; align-items:stretch; }
    .bar-cell{ width:90px; }
  }
  @media (max-width: 520px){
    .stats-row{ grid-template-columns:1fr; }
  }
</style>
  <link rel="icon" type="image/png" href="data:image/png;base64,">
</head>
<body>

<nav>
  <div class="nav-inner">
    <a href="dashboard.html" class="brand">
      <span class="mark"><img src="data:image/png;base64," alt="TrackMySpend logo"></span>
      TrackMySpend
    </a>
    <div class="nav-links">
      <a href="dashboard.html">Dashboard</a>
      <a href="transactions.html">Transactions</a>
      <a href="budgets.html">Budgets</a>
      <a href="goals.html">Goals</a>
      <a href="reports.html" class="active">Reports</a>
      <a href="settings.html">Settings</a>
      <a href="profile.html">Profile</a>
    </div>
    <div class="nav-user">
      <span class="nav-avatar">RS</span>
      <a href="index.html" class="nav-signout">Sign out</a>
    </div>
  </div>
</nav>

<div class="wrap">

  <div class="page-head">
    <div class="kicker">Analytics</div>
    <h1>Reports &amp; analytics</h1>
    <p>Every category, every month, laid out and ready to export.</p>
  </div>

  <!-- FILTER + EXPORT BAR -->
  <div class="filter-bar">
    <div class="filter-group">
      <div class="seg" id="periodToggle">
        <button class="active" data-mode="monthly">Monthly</button>
        <button data-mode="yearly">Yearly</button>
      </div>
      <div class="field-select">
        <span>Period</span>
        <select id="periodValue">
          <option>July 2026</option>
          <option>June 2026</option>
          <option>May 2026</option>
          <option>April 2026</option>
        </select>
      </div>
    </div>
    <div class="export-group">
      <button class="btn btn-ghost" id="exportCsv">⬇ Export CSV</button>
      <button class="btn btn-primary" id="exportPdf">⬇ Download PDF report</button>
    </div>
  </div>

  <!-- SUMMARY CARDS -->
  <div class="stats-row">
    <div class="stat">
      <div class="label">Total income</div>
      <div class="value">₹68,000</div>
      <div class="sub up">↑ 6% vs June</div>
    </div>
    <div class="stat">
      <div class="label">Total expense</div>
      <div class="value">₹41,860</div>
      <div class="sub down">↑ 12% vs June</div>
    </div>
    <div class="stat">
      <div class="label">Net saved</div>
      <div class="value">₹26,140</div>
      <div class="sub">38.4% savings rate</div>
    </div>
    <div class="stat">
      <div class="label">Top category</div>
      <div class="value">Housing</div>
      <div class="sub">₹18,000 · 43% of spend</div>
    </div>
  </div>

  <!-- CHARTS -->
  <div class="chart-grid">
    <div class="panel">
      <div class="panel-head">
        <div><h3>Category-wise breakdown</h3></div>
        <span class="hint">July 2026</span>
      </div>
      <div class="chart-box">
        <canvas id="pieChart"></canvas>
      </div>
      <div class="legend-row" id="pieLegend"></div>
    </div>

    <div class="panel">
      <div class="panel-head">
        <div><h3>Income vs. expense</h3></div>
        <span class="hint">Jan – Jul 2026</span>
      </div>
      <div class="chart-box">
        <canvas id="barChart"></canvas>
      </div>
    </div>
  </div>

  <div class="panel" style="margin-top:24px;">
    <div class="panel-head">
      <div><h3>Spending &amp; saving trend</h3></div>
      <span class="hint">Jan – Jul 2026</span>
    </div>
    <div class="chart-box">
      <canvas id="lineChart"></canvas>
    </div>
  </div>

  <!-- BREAKDOWN TABLE -->
  <div class="panel table-wrap">
    <div class="panel-head">
      <div><h3>Category detail</h3></div>
      <span class="hint">July 2026</span>
    </div>
    <table>
      <thead>
        <tr>
          <th>Category</th>
          <th class="num">Amount</th>
          <th class="num">Share</th>
          <th>Distribution</th>
        </tr>
      </thead>
      <tbody id="categoryTableBody"></tbody>
    </table>
  </div>

  <!-- DOWNLOADABLE REPORTS -->
  <div class="download-grid">
    <div class="download-card">
      <div class="d-kicker">Monthly summary</div>
      <h3>July 2026 report</h3>
      <p>Income, expenses, category breakdown, and budget performance for the month — formatted as a one-page ledger.</p>
      <div class="download-actions">
        <button class="dl-btn gold" id="dlMonthlyPdf">⬇ Download PDF</button>
        <button class="dl-btn" id="dlMonthlyCsv">⬇ Download CSV</button>
      </div>
      <div class="dl-note" id="monthlyNote">● Report generated</div>
    </div>
    <div class="download-card">
      <div class="d-kicker">Yearly summary</div>
      <h3>2026 report</h3>
      <p>Full-year totals, month-by-month trend, and category-wise annual spend — ready for tax season or a year-in-review.</p>
      <div class="download-actions">
        <button class="dl-btn gold" id="dlYearlyPdf">⬇ Download PDF</button>
        <button class="dl-btn" id="dlYearlyCsv">⬇ Download CSV</button>
      </div>
      <div class="dl-note" id="yearlyNote">● Report generated</div>
    </div>
  </div>

</div>

<script>
  /* ---------------------------------------------------------------
     Sample aggregation payload — shaped like the JSON the backend's
     /api/reports/summary?period=2026-07 endpoint would return.
     Swap for a fetch() call once the API is live.
  --------------------------------------------------------------- */
  const reportData = {
    months: ['Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul'],
    income:  [62000, 62000, 64500, 64500, 64000, 68000],
    expense: [38400, 41200, 36900, 44700, 37300, 41860],
    savingsTrend: [23600, 20800, 27600, 19800, 26700, 26140],
    categories: [
      { label: 'Housing',          value: 18000, color: '#2c5842' },
      { label: 'Food & groceries', value: 8420,  color: '#3f7a5c' },
      { label: 'Transport',        value: 4180,  color: '#6fa387' },
      { label: 'Entertainment',    value: 2860,  color: '#c9a15a' },
      { label: 'Utilities',        value: 3600,  color: '#b1462f' },
      { label: 'Other',            value: 4800,  color: '#8a8478' }
    ]
  };

  const paper = '#f4efe1';
  const inkSoft = '#4b5850';
  const gridColor = 'rgba(28,38,32,0.08)';

  Chart.defaults.font.family = "'IBM Plex Sans', sans-serif";
  Chart.defaults.color = inkSoft;

  // Pie / doughnut — category-wise spending breakdown
  new Chart(document.getElementById('pieChart'), {
    type: 'pie',
    data: {
      labels: reportData.categories.map(c => c.label),
      datasets: [{
        data: reportData.categories.map(c => c.value),
        backgroundColor: reportData.categories.map(c => c.color),
        borderColor: paper, borderWidth: 3
      }]
    },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } }
  });

  const legendEl = document.getElementById('pieLegend');
  const total = reportData.categories.reduce((s,c) => s + c.value, 0);
  reportData.categories.forEach(c => {
    const pct = Math.round((c.value / total) * 100);
    const item = document.createElement('div');
    item.className = 'legend-item';
    item.innerHTML = `<span class="legend-dot" style="background:${c.color}"></span>${c.label} · ${pct}%`;
    legendEl.appendChild(item);
  });

  // Bar chart — monthly income vs expense comparison
  new Chart(document.getElementById('barChart'), {
    type: 'bar',
    data: {
      labels: reportData.months,
      datasets: [
        { label: 'Income', data: reportData.income, backgroundColor: '#3f7a5c', borderRadius: 4, maxBarThickness: 26 },
        { label: 'Expense', data: reportData.expense, backgroundColor: '#c9a15a', borderRadius: 4, maxBarThickness: 26 }
      ]
    },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { position: 'top', align: 'end', labels: { boxWidth: 10, boxHeight: 10, usePointStyle: true, pointStyle: 'circle' } } },
      scales: {
        y: { grid: { color: gridColor }, ticks: { callback: (v) => '₹' + (v/1000) + 'k' } },
        x: { grid: { display: false } }
      }
    }
  });

  // Line chart — spending / saving trend over time
  new Chart(document.getElementById('lineChart'), {
    type: 'line',
    data: {
      labels: reportData.months,
      datasets: [
        {
          label: 'Net saved', data: reportData.savingsTrend,
          borderColor: '#2c5842', backgroundColor: 'rgba(63,122,92,0.12)',
          fill: true, tension: 0.35, pointRadius: 3, pointBackgroundColor: '#2c5842'
        },
        {
          label: 'Expense', data: reportData.expense,
          borderColor: '#b1462f', backgroundColor: 'transparent', borderDash: [4,4],
          fill: false, tension: 0.35, pointRadius: 2, pointBackgroundColor: '#b1462f'
        }
      ]
    },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { position: 'top', align: 'end', labels: { boxWidth: 10, boxHeight: 10, usePointStyle: true, pointStyle: 'circle' } } },
      scales: {
        y: { grid: { color: gridColor }, ticks: { callback: (v) => '₹' + (v/1000) + 'k' } },
        x: { grid: { display: false } }
      }
    }
  });

  // Category detail table
  const tbody = document.getElementById('categoryTableBody');
  reportData.categories
    .slice()
    .sort((a,b) => b.value - a.value)
    .forEach(c => {
      const pct = Math.round((c.value / total) * 100);
      const tr = document.createElement('tr');
      tr.innerHTML = `
        <td><span class="cat-name"><span class="cat-dot" style="background:${c.color}"></span>${c.label}</span></td>
        <td class="num">₹${c.value.toLocaleString('en-IN')}</td>
        <td class="num">${pct}%</td>
        <td class="bar-cell"><div class="mini-bar"><div class="fill" style="width:${pct}%; background:${c.color}"></div></div></td>
      `;
      tbody.appendChild(tr);
    });

  // Monthly / Yearly toggle (purely visual — swaps the period label)
  const toggleButtons = document.querySelectorAll('#periodToggle button');
  const periodValue = document.getElementById('periodValue');
  toggleButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      toggleButtons.forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      periodValue.innerHTML = btn.dataset.mode === 'yearly'
        ? '<option>2026</option><option>2025</option>'
        : '<option>July 2026</option><option>June 2026</option><option>May 2026</option><option>April 2026</option>';
    });
  });

  // CSV export — builds a real downloadable file from the aggregation data
  function buildCsv() {
    const rows = [['Category', 'Amount (INR)', 'Share']];
    reportData.categories.forEach(c => {
      rows.push([c.label, c.value, Math.round((c.value/total)*100) + '%']);
    });
    return rows.map(r => r.join(',')).join('\n');
  }
  function downloadFile(filename, content, type) {
    const blob = new Blob([content], { type });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = filename;
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
    URL.revokeObjectURL(url);
  }
  function flashNote(el) {
    el.classList.add('show');
    clearTimeout(el._t);
    el._t = setTimeout(() => el.classList.remove('show'), 2400);
  }

  document.getElementById('exportCsv').addEventListener('click', () => downloadFile('trackmyspend-category-breakdown.csv', buildCsv(), 'text/csv'));
  document.getElementById('dlMonthlyCsv').addEventListener('click', () => { downloadFile('trackmyspend-monthly-summary-jul-2026.csv', buildCsv(), 'text/csv'); flashNote(document.getElementById('monthlyNote')); });
  document.getElementById('dlYearlyCsv').addEventListener('click', () => { downloadFile('trackmyspend-yearly-summary-2026.csv', buildCsv(), 'text/csv'); flashNote(document.getElementById('yearlyNote')); });

  // PDF export — uses the browser's print dialog (Save as PDF) against this report view
  document.getElementById('exportPdf').addEventListener('click', () => window.print());
  document.getElementById('dlMonthlyPdf').addEventListener('click', () => { flashNote(document.getElementById('monthlyNote')); window.print(); });
  document.getElementById('dlYearlyPdf').addEventListener('click', () => { flashNote(document.getElementById('yearlyNote')); window.print(); });
</script>

</body>
</html>
