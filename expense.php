<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Add expense — TrackMySpend</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,400;0,9..144,500;0,9..144,600;0,9..144,700;1,9..144,500&family=IBM+Plex+Mono:wght@400;500;600&family=IBM+Plex+Sans:wght@400;500;600&display=swap" rel="stylesheet">
<style>
  :root{
    --cover: #101c16;
    --cover-deep: #0a1410;
    --paper: #f4efe1;
    --paper-dim: #ece5d2;
    --ink: #1c2620;
    --ink-soft: #4b5850;
    --money: #3f7a5c;
    --money-deep: #2c5842;
    --stamp: #b1462f;
    --gold: #c9a15a;
    --line: rgba(28,38,32,0.14);
    --line-on-cover: rgba(244,239,225,0.16);
  }

  *{ margin:0; padding:0; box-sizing:border-box; }
  html,body{ min-height:100%; }
  body{
    background:var(--paper);
    color:var(--ink);
    font-family:'IBM Plex Sans', sans-serif;
    -webkit-font-smoothing:antialiased;
  }
  @media (prefers-reduced-motion: reduce){
    *{ animation-duration:0.001ms !important; animation-iteration-count:1 !important; transition-duration:0.001ms !important; }
  }
  a{ color:inherit; text-decoration:none; }
  .display{ font-family:'Fraunces', serif; }
  .mono{ font-family:'IBM Plex Mono', monospace; }
  ::selection{ background:var(--gold); color:var(--cover-deep); }

  .wrap{ max-width:1080px; margin:0 auto; padding:0 32px; }

  /* ---------------- NAV ---------------- */
  nav{ background:var(--cover); border-bottom:1px solid var(--line-on-cover); }
  .nav-inner{ display:flex; align-items:center; justify-content:space-between; padding:16px 32px; max-width:1080px; margin:0 auto; }
  .brand{ display:flex; align-items:center; gap:11px; font-family:'Fraunces', serif; font-weight:600; font-size:1.1rem; color:var(--paper); }
  .brand .mark{ width:32px; height:32px; border-radius:7px; overflow:hidden; flex:none; background:var(--paper); box-shadow:0 2px 0 var(--money-deep); display:flex; align-items:center; justify-content:center; }
  .brand .mark img{ width:100%; height:100%; object-fit:cover; display:block; }
  .nav-links{ display:flex; gap:20px; font-size:0.86rem; }
  .nav-links a{ position:relative; color:rgba(244,239,225,0.68); transition:color 0.2s; padding-bottom:4px; }
  .nav-links a::after{ content:''; position:absolute; left:0; right:0; bottom:0; height:2px; background:var(--gold); transform:scaleX(0); transform-origin:left; transition:transform 0.25s ease; }
  .nav-links a:hover{ color:var(--paper); }
  .nav-links a:hover::after{ transform:scaleX(1); }
  .nav-links a.active{ color:var(--paper); }
  .nav-links a.active::after{ transform:scaleX(1); }
  .nav-user{ display:flex; align-items:center; gap:10px; }
  .nav-avatar{ width:32px; height:32px; border-radius:50%; background:var(--money); color:var(--paper); display:flex; align-items:center; justify-content:center; font-family:'IBM Plex Mono', monospace; font-weight:600; font-size:0.78rem; border:1.5px solid var(--gold); }
  .nav-signout{ font-size:0.82rem; color:rgba(244,239,225,0.55); border:1px solid var(--line-on-cover); padding:7px 13px; border-radius:5px; transition:opacity 0.2s; }
  .nav-signout:hover{ opacity:0.7; }
  @media (max-width: 760px){ .nav-links{ display:none; } }

  /* ---------------- PAGE HEAD ---------------- */
  .page-head{ padding:44px 0 0; display:flex; align-items:flex-end; justify-content:space-between; gap:20px; flex-wrap:wrap; }
  .kicker{ font-family:'IBM Plex Mono', monospace; font-size:0.72rem; letter-spacing:0.1em; text-transform:uppercase; color:var(--stamp); margin-bottom:8px; }
  .page-head h1{ font-family:'Fraunces', serif; font-weight:600; font-size:2rem; }
  .page-head p{ margin-top:8px; color:var(--ink-soft); font-size:0.96rem; }
  .back-link{ font-size:0.84rem; color:var(--ink-soft); border-bottom:1px solid var(--line); padding-bottom:2px; white-space:nowrap; }
  .back-link:hover{ color:var(--ink); }

  /* ---------------- MAIN GRID ---------------- */
  .main-grid{ margin:32px 0 80px; display:grid; grid-template-columns: 1.3fr 1fr; gap:24px; align-items:start; }
  @media (max-width: 860px){ .main-grid{ grid-template-columns:1fr; } }

  .panel{ background:#fff; border:1px solid var(--line); border-radius:10px; padding:28px 30px; }
  .panel + .panel{ margin-top:24px; }
  .panel-head{ display:flex; align-items:center; justify-content:space-between; margin-bottom:22px; gap:12px; }
  .panel-head h3{ font-family:'Fraunces', serif; font-weight:600; font-size:1.18rem; }
  .panel-head .hint{ font-family:'IBM Plex Mono', monospace; font-size:0.7rem; color:var(--ink-soft); }

  .field{ display:flex; flex-direction:column; gap:8px; margin-bottom:18px; }
  .field:last-child{ margin-bottom:0; }
  .field-row{ display:grid; grid-template-columns:1fr 1fr; gap:14px; }
  .field label{ font-family:'IBM Plex Mono', monospace; font-size:0.7rem; letter-spacing:0.06em; text-transform:uppercase; color:var(--ink-soft); }
  .field label .req{ color:var(--stamp); }

  input[type="text"], input[type="number"], input[type="date"], select, textarea{
    width:100%; padding:12px 13px; border-radius:6px; border:1px solid var(--line); background:#fff;
    font-family:'IBM Plex Sans', sans-serif; font-size:0.92rem; color:var(--ink);
    transition:border-color 0.15s ease, box-shadow 0.15s ease;
  }
  textarea{ resize:vertical; min-height:80px; font-family:inherit; }
  input:focus, select:focus, textarea:focus{ outline:none; border-color:var(--money); box-shadow:0 0 0 3px rgba(63,122,92,0.14); }
  input.err, select.err{ border-color:var(--stamp); }
  .hint-err{ font-family:'IBM Plex Mono', monospace; font-size:0.74rem; color:var(--stamp); min-height:1em; }

  .amount-wrap{ position:relative; }
  .amount-wrap .cur{ position:absolute; left:13px; top:50%; transform:translateY(-50%); color:var(--ink-soft); font-family:'IBM Plex Mono', monospace; font-size:0.92rem; pointer-events:none; }
  .amount-wrap input{ padding-left:28px; }

  .cat-row{ display:flex; gap:8px; }
  .cat-row select{ flex:1; }
  .cat-add-btn{ flex:none; width:44px; border-radius:6px; border:1px solid var(--line); background:var(--paper-dim); color:var(--ink-soft); font-size:1.1rem; cursor:pointer; transition:background 0.15s; }
  .cat-add-btn:hover{ background:var(--line); color:var(--ink); }

  .save-row{ display:flex; justify-content:flex-end; gap:12px; margin-top:22px; align-items:center; flex-wrap:wrap; }
  .btn{ padding:11px 20px; border-radius:6px; font-weight:600; font-size:0.88rem; border:none; cursor:pointer; transition:transform 0.15s ease, box-shadow 0.15s ease; }
  .btn-primary{ background:var(--stamp); color:var(--paper); box-shadow:0 3px 0 #7c2f1e; }
  .btn-primary:hover{ transform:translateY(-1px); box-shadow:0 4px 0 #7c2f1e; }
  .btn-primary:active{ transform:translateY(1px); box-shadow:0 1px 0 #7c2f1e; }
  .btn-ghost{ background:none; border:1px solid var(--line); color:var(--ink-soft); }
  .btn-ghost:hover{ background:var(--paper-dim); }
  .btn-danger-text{ background:none; border:none; color:var(--stamp); font-size:0.84rem; font-weight:600; cursor:pointer; margin-right:auto; }
  .btn-danger-text:hover{ text-decoration:underline; }

  .save-note{ font-family:'IBM Plex Mono', monospace; font-size:0.76rem; color:var(--money-deep); display:flex; align-items:center; gap:6px; opacity:0; transition:opacity 0.3s ease; }
  .save-note.show{ opacity:1; }
  .save-note .dot{ width:6px; height:6px; border-radius:50%; background:var(--money); }

  /* ---------------- SIDE: quick totals + recent ---------------- */
  .stat-mini{ display:flex; flex-direction:column; gap:4px; padding:16px 0; border-bottom:1px solid var(--line); }
  .stat-mini:last-child{ border-bottom:none; padding-bottom:0; }
  .stat-mini .label{ font-family:'IBM Plex Mono', monospace; font-size:0.68rem; letter-spacing:0.06em; text-transform:uppercase; color:var(--ink-soft); }
  .stat-mini .value{ font-family:'Fraunces', serif; font-weight:600; font-size:1.4rem; }

  .tx-item{ display:flex; align-items:center; justify-content:space-between; gap:10px; padding:12px 0; border-bottom:1px solid var(--line); }
  .tx-item:last-child{ border-bottom:none; padding-bottom:0; }
  .tx-left{ display:flex; align-items:center; gap:12px; min-width:0; }
  .tx-icon{ width:34px; height:34px; border-radius:8px; background:var(--paper-dim); display:flex; align-items:center; justify-content:center; font-size:0.9rem; flex:none; }
  .tx-info{ min-width:0; }
  .tx-info .t-cat{ font-size:0.88rem; font-weight:600; color:var(--ink); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
  .tx-info .t-sub{ font-size:0.76rem; color:var(--ink-soft); margin-top:2px; }
  .tx-right{ display:flex; align-items:center; gap:10px; flex:none; }
  .tx-amount{ font-family:'IBM Plex Mono', monospace; font-weight:600; font-size:0.9rem; color:var(--stamp); }
  .tx-actions{ display:flex; gap:6px; }
  .icon-btn{ width:26px; height:26px; border-radius:5px; border:1px solid var(--line); background:#fff; color:var(--ink-soft); font-size:0.76rem; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:background 0.15s; }
  .icon-btn:hover{ background:var(--paper-dim); }
  .icon-btn.danger:hover{ background:#fbe6e0; color:var(--stamp); border-color:var(--stamp); }
  .empty-note{ font-size:0.86rem; color:var(--ink-soft); padding:8px 0; }

  /* ---------------- MODAL ---------------- */
  .modal-overlay{ position:fixed; inset:0; background:rgba(16,28,22,0.55); display:none; align-items:center; justify-content:center; z-index:100; padding:20px; }
  .modal-overlay.show{ display:flex; }
  .modal-card{ background:#fff; border-radius:10px; padding:26px 26px 22px; max-width:360px; width:100%; box-shadow:0 20px 50px rgba(0,0,0,0.25); }
  .modal-card h4{ font-family:'Fraunces', serif; font-weight:600; font-size:1.1rem; margin-bottom:8px; }
  .modal-card p{ font-size:0.88rem; color:var(--ink-soft); line-height:1.5; }
  .modal-actions{ display:flex; justify-content:flex-end; gap:10px; margin-top:20px; }
  .btn-danger{ background:var(--stamp); color:var(--paper); }
  .btn-danger:hover{ opacity:0.92; }

  .toast{ position:fixed; bottom:24px; left:50%; transform:translateX(-50%) translateY(20px); background:var(--cover); color:var(--paper); padding:12px 20px; border-radius:8px; font-size:0.86rem; display:flex; align-items:center; gap:8px; opacity:0; pointer-events:none; transition:opacity 0.25s ease, transform 0.25s ease; z-index:200; }
  .toast.show{ opacity:1; transform:translateX(-50%) translateY(0); }
  .toast .dot{ width:6px; height:6px; border-radius:50%; background:var(--money); }
</style>
  <link rel="icon" type="image/png" href="data:image/png;base64,">
</head>
<body>

<nav>
  <div class="nav-inner">
    <a href="dashboard.html" class="brand">
      <div class="mark"><img src="data:image/png;base64," alt="TrackMySpend logo"></div>
      TrackMySpend
    </a>
    <div class="nav-links">
      <a href="dashboard.html">Dashboard</a>
      <a href="transactions.html" class="active">Transactions</a>
      <a href="budgets.html">Budgets</a>
      <a href="goals.html">Goals</a>
      <a href="reports.html">Reports</a>
      <a href="settings.html">Settings</a>
      <a href="profile.html">Profile</a>
    </div>
    <div class="nav-user">
      <span class="nav-avatar">RS</span>
      <a href="index.html" class="nav-signout">Sign out</a>
    </div>
  </div>
</nav>

<div class="wrap">

  <div class="page-head">
    <div>
      <div class="kicker">Expense entry</div>
      <h1 id="pageTitle">Add an expense</h1>
      <p id="pageSub">Log where the money went — it lands straight in your transaction history.</p>
    </div>
    <a href="transactions.html" class="back-link">← Back to transactions</a>
  </div>

  <div class="main-grid">

    <!-- LEFT: FORM -->
    <div class="panel">
      <div class="panel-head">
        <h3 id="formHeading">Expense details</h3>
        <span class="hint" id="formHint">All fields marked * are required</span>
      </div>

      <form id="expenseForm" novalidate>
        <input type="hidden" id="expenseId" value="">

        <div class="field-row">
          <div class="field">
            <label for="amount">Amount *</label>
            <div class="amount-wrap">
              <span class="cur">₹</span>
              <input type="number" id="amount" name="amount" placeholder="0.00" step="0.01" min="0.01" required>
            </div>
            <span class="hint-err" id="amountErr"></span>
          </div>
          <div class="field">
            <label for="date">Date *</label>
            <input type="date" id="date" name="date" required>
            <span class="hint-err" id="dateErr"></span>
          </div>
        </div>

        <div class="field-row">
          <div class="field">
            <label for="category">Category *</label>
            <div class="cat-row">
              <select id="category" name="category" required></select>
              <button type="button" class="cat-add-btn" id="addCatBtn" title="Add a new category">+</button>
            </div>
            <span class="hint-err" id="categoryErr"></span>
          </div>
          <div class="field">
            <label for="paymentMethod">Payment method</label>
            <select id="paymentMethod" name="paymentMethod">
              <option>UPI</option>
              <option>Debit card</option>
              <option>Credit card</option>
              <option>Cash</option>
              <option>Bank transfer</option>
              <option>Other</option>
            </select>
          </div>
        </div>

        <div class="field">
          <label for="note">Note</label>
          <input type="text" id="note" name="note" placeholder="e.g. Groceries for the week, dinner with friends">
        </div>

        <div class="save-row">
          <button type="button" class="btn-danger-text" id="deleteBtn" style="display:none;">Delete expense</button>
          <span class="save-note" id="saveNote"><span class="dot"></span>Expense saved</span>
          <button type="button" class="btn btn-ghost" id="resetBtn">Cancel</button>
          <button type="submit" class="btn btn-primary" id="submitBtn">Save expense</button>
        </div>
      </form>
    </div>

    <!-- RIGHT: SUMMARY + RECENT -->
    <div>
      <div class="panel">
        <div class="panel-head"><h3>This month</h3></div>
        <div class="stat-mini">
          <div class="label">Total expenses logged</div>
          <div class="value" id="totalCount">0</div>
        </div>
        <div class="stat-mini">
          <div class="label">Total spent</div>
          <div class="value" id="totalAmount">₹0</div>
        </div>
      </div>

      <div class="panel">
        <div class="panel-head">
          <h3>Recent expenses</h3>
          <span class="hint">Latest 5</span>
        </div>
        <div id="recentList"></div>
      </div>
    </div>

  </div>
</div>

<!-- Delete confirmation modal -->
<div class="modal-overlay" id="deleteModal">
  <div class="modal-card">
    <h4>Delete this expense?</h4>
    <p>This removes it from your transaction history for good. This can't be undone.</p>
    <div class="modal-actions">
      <button type="button" class="btn btn-ghost" id="cancelDelete">Cancel</button>
      <button type="button" class="btn btn-danger" id="confirmDelete">Delete</button>
    </div>
  </div>
</div>

<div class="toast" id="toast"><span class="dot"></span><span id="toastText">Saved</span></div>

<script>
  // ---------- storage layer (stands in for the expenses table until the backend is wired up) ----------
  const EXPENSES_KEY = 'trackmyspend_expenses';
  const CATEGORIES_KEY = 'trackmyspend_categories';

  const DEFAULT_CATEGORIES = [
    'Groceries', 'Food & Dining', 'Transport', 'Bills & Utilities',
    'Shopping', 'Entertainment', 'Health', 'Rent', 'Other'
  ];

  function loadCategories(){
    let cats = JSON.parse(localStorage.getItem(CATEGORIES_KEY) || 'null');
    if(!cats || !cats.length){
      cats = DEFAULT_CATEGORIES;
      localStorage.setItem(CATEGORIES_KEY, JSON.stringify(cats));
    }
    return cats;
  }
  function saveCategories(cats){ localStorage.setItem(CATEGORIES_KEY, JSON.stringify(cats)); }

  function loadExpenses(){ return JSON.parse(localStorage.getItem(EXPENSES_KEY) || '[]'); }
  function saveExpenses(list){ localStorage.setItem(EXPENSES_KEY, JSON.stringify(list)); }

  function uid(){ return 'exp_' + Date.now() + '_' + Math.random().toString(36).slice(2,8); }

  function formatINR(n){
    return '₹' + Number(n).toLocaleString('en-IN', { maximumFractionDigits: 2 });
  }

  // ---------- populate category dropdown ----------
  const categorySelect = document.getElementById('category');
  function renderCategoryOptions(selected){
    const cats = loadCategories();
    categorySelect.innerHTML = '<option value="" disabled ' + (selected ? '' : 'selected') + '>Choose a category</option>' +
      cats.map(c => `<option value="${c}" ${c === selected ? 'selected' : ''}>${c}</option>`).join('');
  }
  renderCategoryOptions();

  document.getElementById('addCatBtn').addEventListener('click', () => {
    const name = prompt('New category name');
    if(name && name.trim()){
      const cats = loadCategories();
      if(!cats.includes(name.trim())){
        cats.push(name.trim());
        saveCategories(cats);
      }
      renderCategoryOptions(name.trim());
    }
  });

  // ---------- form state ----------
  const form = document.getElementById('expenseForm');
  const idField = document.getElementById('expenseId');
  const amountField = document.getElementById('amount');
  const dateField = document.getElementById('date');
  const noteField = document.getElementById('note');
  const paymentField = document.getElementById('paymentMethod');
  const deleteBtn = document.getElementById('deleteBtn');
  const saveNote = document.getElementById('saveNote');
  const resetBtn = document.getElementById('resetBtn');
  const submitBtn = document.getElementById('submitBtn');

  const params = new URLSearchParams(window.location.search);
  const editId = params.get('id');

  function today(){ return new Date().toISOString().slice(0,10); }

  function loadForEdit(id){
    const list = loadExpenses();
    const exp = list.find(e => e.id === id);
    if(!exp) return false;
    idField.value = exp.id;
    amountField.value = exp.amount;
    dateField.value = exp.date;
    renderCategoryOptions(exp.category);
    paymentField.value = exp.paymentMethod || 'UPI';
    noteField.value = exp.note || '';
    document.getElementById('pageTitle').textContent = 'Edit expense';
    document.getElementById('pageSub').textContent = 'Update the details — changes save straight to your transaction history.';
    document.getElementById('formHeading').textContent = 'Edit expense details';
    submitBtn.textContent = 'Update expense';
    deleteBtn.style.display = 'inline-block';
    return true;
  }

  if(editId){
    const ok = loadForEdit(editId);
    if(!ok) dateField.value = today();
  } else {
    dateField.value = today();
  }

  function clearErrors(){
    [amountField, dateField, categorySelect].forEach(f => f.classList.remove('err'));
    ['amountErr','dateErr','categoryErr'].forEach(id => document.getElementById(id).textContent = '');
  }

  function showToast(text){
    const toast = document.getElementById('toast');
    document.getElementById('toastText').textContent = text;
    toast.classList.add('show');
    clearTimeout(showToast._t);
    showToast._t = setTimeout(() => toast.classList.remove('show'), 2400);
  }

  function refreshSummary(){
    const list = loadExpenses();
    const now = new Date();
    const monthKey = now.toISOString().slice(0,7);
    const monthly = list.filter(e => e.date && e.date.slice(0,7) === monthKey);
    document.getElementById('totalCount').textContent = monthly.length;
    const total = monthly.reduce((sum, e) => sum + Number(e.amount || 0), 0);
    document.getElementById('totalAmount').textContent = formatINR(total);
  }

  const CATEGORY_ICONS = {
    'Groceries': '🛒', 'Food & Dining': '🍽️', 'Transport': '🚌', 'Bills & Utilities': '💡',
    'Shopping': '🛍️', 'Entertainment': '🎬', 'Health': '💊', 'Rent': '🏠', 'Other': '📦'
  };

  let pendingDeleteId = null;

  function renderRecent(){
    const list = loadExpenses().slice().sort((a,b) => (b.createdAt || 0) - (a.createdAt || 0)).slice(0,5);
    const container = document.getElementById('recentList');
    if(!list.length){
      container.innerHTML = '<div class="empty-note">No expenses logged yet — the one you save will show up here.</div>';
      return;
    }
    container.innerHTML = list.map(e => `
      <div class="tx-item">
        <div class="tx-left">
          <div class="tx-icon">${CATEGORY_ICONS[e.category] || '📦'}</div>
          <div class="tx-info">
            <div class="t-cat">${e.category}${e.note ? ' · ' + e.note : ''}</div>
            <div class="t-sub">${e.date} · ${e.paymentMethod || ''}</div>
          </div>
        </div>
        <div class="tx-right">
          <span class="tx-amount">−${formatINR(e.amount)}</span>
          <div class="tx-actions">
            <button class="icon-btn" title="Edit" onclick="window.location.href='expense.html?id=${e.id}'">✎</button>
            <button class="icon-btn danger" title="Delete" onclick="openDeleteModal('${e.id}')">✕</button>
          </div>
        </div>
      </div>
    `).join('');
  }

  window.openDeleteModal = function(id){
    pendingDeleteId = id;
    document.getElementById('deleteModal').classList.add('show');
  };
  document.getElementById('cancelDelete').addEventListener('click', () => {
    pendingDeleteId = null;
    document.getElementById('deleteModal').classList.remove('show');
  });
  document.getElementById('confirmDelete').addEventListener('click', () => {
    if(pendingDeleteId){
      const list = loadExpenses().filter(e => e.id !== pendingDeleteId);
      saveExpenses(list);
      showToast('Expense deleted');
      renderRecent();
      refreshSummary();
      if(idField.value === pendingDeleteId){
        form.reset();
        idField.value = '';
        window.history.replaceState({}, '', 'expense.html');
        location.reload();
      }
    }
    pendingDeleteId = null;
    document.getElementById('deleteModal').classList.remove('show');
  });

  deleteBtn.addEventListener('click', () => {
    if(idField.value) window.openDeleteModal(idField.value);
  });

  resetBtn.addEventListener('click', () => {
    window.location.href = 'expense.html';
  });

  form.addEventListener('submit', (e) => {
    e.preventDefault();
    clearErrors();
    let valid = true;

    if(!amountField.value || Number(amountField.value) <= 0){
      amountField.classList.add('err');
      document.getElementById('amountErr').textContent = 'Enter an amount greater than 0.';
      valid = false;
    }
    if(!dateField.value){
      dateField.classList.add('err');
      document.getElementById('dateErr').textContent = 'Pick a date for this expense.';
      valid = false;
    }
    if(!categorySelect.value){
      categorySelect.classList.add('err');
      document.getElementById('categoryErr').textContent = 'Choose a category.';
      valid = false;
    }
    if(!valid) return;

    const list = loadExpenses();
    const isEdit = !!idField.value;
    const record = {
      id: isEdit ? idField.value : uid(),
      amount: Number(amountField.value),
      date: dateField.value,
      category: categorySelect.value,
      paymentMethod: paymentField.value,
      note: noteField.value.trim(),
      createdAt: isEdit ? (list.find(x => x.id === idField.value)?.createdAt || Date.now()) : Date.now()
    };

    if(isEdit){
      const idx = list.findIndex(x => x.id === record.id);
      if(idx > -1) list[idx] = record; else list.push(record);
    } else {
      list.push(record);
    }
    saveExpenses(list);

    saveNote.classList.add('show');
    clearTimeout(form._t);
    form._t = setTimeout(() => saveNote.classList.remove('show'), 2600);
    showToast(isEdit ? 'Expense updated' : 'Expense saved');

    renderRecent();
    refreshSummary();

    if(!isEdit){
      form.reset();
      dateField.value = today();
      renderCategoryOptions();
      amountField.focus();
    }
  });

  renderRecent();
  refreshSummary();
</script>

</body>
</html>
