<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Settings — TrackMySpend</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,400;0,9..144,500;0,9..144,600;0,9..144,700;1,9..144,500&family=IBM+Plex+Mono:wght@400;500;600&family=IBM+Plex+Sans:wght@400;500;600&display=swap" rel="stylesheet">
<style>
  :root{
    --cover: #101c16;
    --cover-deep: #0a1410;
    --paper: #f4efe1;
    --paper-dim: #ece5d2;
    --ink: #1c2620;
    --ink-soft: #4b5850;
    --money: #3f7a5c;
    --money-deep: #2c5842;
    --stamp: #b1462f;
    --gold: #c9a15a;
    --line: rgba(28,38,32,0.14);
    --line-on-cover: rgba(244,239,225,0.16);
  }
  *{ margin:0; padding:0; box-sizing:border-box; }
  html,body{ min-height:100%; }
  body{
    background:var(--paper);
    color:var(--ink);
    font-family:'IBM Plex Sans', sans-serif;
    -webkit-font-smoothing:antialiased;
  }
  @media (prefers-reduced-motion: reduce){
    *{ animation-duration:0.001ms !important; animation-iteration-count:1 !important; transition-duration:0.001ms !important; }
  }
  a{ color:inherit; text-decoration:none; }
  .display{ font-family:'Fraunces', serif; }
  .mono{ font-family:'IBM Plex Mono', monospace; }
  ::selection{ background:var(--gold); color:var(--cover-deep); }

  .wrap{ max-width:1160px; margin:0 auto; padding:0 32px; }

  nav{ background:var(--cover); border-bottom:1px solid var(--line-on-cover); position:sticky; top:0; z-index:60; }
  .nav-inner{ display:flex; align-items:center; justify-content:space-between; padding:16px 32px; max-width:1160px; margin:0 auto; }
  .brand{ display:flex; align-items:center; gap:11px; font-family:'Fraunces', serif; font-weight:600; font-size:1.1rem; color:var(--paper); }
  .brand .mark{ width:32px; height:32px; border-radius:7px; overflow:hidden; flex:none; background:var(--paper); box-shadow:0 2px 0 var(--money-deep); }
  .brand .mark img{ width:100%; height:100%; object-fit:cover; display:block; }
  .nav-links{ display:flex; gap:20px; font-size:0.86rem; }
  .nav-links a{ position:relative; color:rgba(244,239,225,0.68); transition:color 0.2s; padding-bottom:4px; }
  .nav-links a::after{ content:''; position:absolute; left:0; right:0; bottom:0; height:2px; background:var(--gold); transform:scaleX(0); transform-origin:left; transition:transform 0.25s ease; }
  .nav-links a:hover{ color:var(--paper); }
  .nav-links a:hover::after{ transform:scaleX(1); }
  .nav-links a.active{ color:var(--paper); }
  .nav-links a.active::after{ transform:scaleX(1); }
  .nav-user{ display:flex; align-items:center; gap:10px; }
  .nav-avatar{ width:32px; height:32px; border-radius:50%; background:var(--money); color:var(--paper); display:flex; align-items:center; justify-content:center; font-family:'IBM Plex Mono', monospace; font-weight:600; font-size:0.78rem; border:1.5px solid var(--gold); }
  .nav-signout{ font-size:0.82rem; color:rgba(244,239,225,0.55); border:1px solid var(--line-on-cover); padding:7px 13px; border-radius:5px; transition:opacity 0.2s; }
  .nav-signout:hover{ opacity:0.7; }
  @media (max-width: 760px){ .nav-links{ display:none; } }

  .page-head{ padding:40px 0 0; }
  .kicker{ font-family:'IBM Plex Mono', monospace; font-size:0.72rem; letter-spacing:0.1em; text-transform:uppercase; color:var(--money-deep); margin-bottom:8px; }
  .page-head h1{ font-family:'Fraunces', serif; font-weight:600; font-size:2rem; }
  .page-head p{ margin-top:8px; color:var(--ink-soft); font-size:0.96rem; max-width:560px; }

  .settings-shell{ margin:32px 0 90px; display:grid; grid-template-columns:230px 1fr; gap:28px; align-items:start; }

  .side-nav{ position:sticky; top:84px; background:#fff; border:1px solid var(--line); border-radius:10px; padding:10px; }
  .side-nav a{ display:flex; align-items:center; gap:10px; padding:11px 14px; border-radius:7px; font-size:0.88rem; color:var(--ink-soft); font-weight:500; transition:background 0.15s, color 0.15s; }
  .side-nav a .ico{ width:18px; text-align:center; font-size:0.9rem; flex:none; }
  .side-nav a:hover{ background:var(--paper-dim); color:var(--ink); }
  .side-nav a.active{ background:var(--cover); color:var(--paper); }
  .side-nav a.active .ico{ color:var(--gold); }
  .side-nav .danger-link{ color:var(--stamp); margin-top:6px; border-top:1px dashed var(--line); padding-top:14px; border-radius:0 0 7px 7px; }
  .side-nav .danger-link:hover{ background:rgba(177,70,47,0.06); }

  .settings-main section{ scroll-margin-top:90px; }
  .settings-main section + section{ margin-top:26px; }

  .panel{ background:#fff; border:1px solid var(--line); border-radius:10px; padding:28px 30px; }
  .panel-head{ display:flex; align-items:flex-start; justify-content:space-between; margin-bottom:22px; gap:16px; }
  .panel-head h3{ font-family:'Fraunces', serif; font-weight:600; font-size:1.18rem; }
  .panel-head .sub{ font-size:0.82rem; color:var(--ink-soft); margin-top:4px; }
  .panel-head .hint{ font-family:'IBM Plex Mono', monospace; font-size:0.7rem; color:var(--ink-soft); white-space:nowrap; }

  .field{ display:flex; flex-direction:column; gap:8px; margin-bottom:18px; }
  .field:last-child{ margin-bottom:0; }
  .field-row{ display:grid; grid-template-columns:1fr 1fr; gap:14px; }
  .field-row3{ display:grid; grid-template-columns:1fr 1fr 1fr; gap:14px; }
  .field label{ font-family:'IBM Plex Mono', monospace; font-size:0.7rem; letter-spacing:0.06em; text-transform:uppercase; color:var(--ink-soft); }
  input[type="text"], input[type="email"], input[type="tel"], input[type="number"], select{
    width:100%; padding:12px 13px; border-radius:6px; border:1px solid var(--line); background:#fff;
    font-family:'IBM Plex Sans', sans-serif; font-size:0.92rem; color:var(--ink);
    transition:border-color 0.15s ease, box-shadow 0.15s ease;
  }
  input:focus, select:focus{ outline:none; border-color:var(--money); box-shadow:0 0 0 3px rgba(63,122,92,0.14); }

  .save-row{ display:flex; justify-content:flex-end; gap:12px; margin-top:22px; align-items:center; }
  .btn{ padding:11px 20px; border-radius:6px; font-weight:600; font-size:0.88rem; border:none; cursor:pointer; transition:transform 0.15s ease, box-shadow 0.15s ease; font-family:inherit; }
  .btn-primary{ background:var(--money-deep); color:var(--paper); box-shadow:0 3px 0 #1e3f2f; }
  .btn-primary:hover{ transform:translateY(-1px); box-shadow:0 4px 0 #1e3f2f; }
  .btn-primary:active{ transform:translateY(1px); box-shadow:0 1px 0 #1e3f2f; }
  .btn-ghost{ background:none; border:1px solid var(--line); color:var(--ink-soft); }
  .btn-ghost:hover{ background:var(--paper-dim); }
  .btn-outline{ background:#fff; border:1px solid var(--money-deep); color:var(--money-deep); }
  .btn-outline:hover{ background:rgba(63,122,92,0.06); }
  .btn-sm{ padding:8px 14px; font-size:0.8rem; border-radius:6px; }

  .save-note{ font-family:'IBM Plex Mono', monospace; font-size:0.76rem; color:var(--money-deep); display:flex; align-items:center; gap:6px; margin-right:auto; opacity:0; transition:opacity 0.3s ease; }
  .save-note.show{ opacity:1; }
  .save-note .dot{ width:6px; height:6px; border-radius:50%; background:var(--money); }

  .toggle-row{ display:flex; align-items:center; justify-content:space-between; padding:14px 0; border-bottom:1px solid var(--line); gap:16px; }
  .toggle-row:last-child{ border-bottom:none; }
  .toggle-row .t-label{ font-size:0.92rem; font-weight:600; }
  .toggle-row .t-sub{ font-size:0.78rem; color:var(--ink-soft); margin-top:2px; }
  .switch{ position:relative; width:42px; height:24px; flex:none; }
  .switch input{ opacity:0; width:0; height:0; }
  .slider{ position:absolute; cursor:pointer; inset:0; background:var(--line); border-radius:24px; transition:0.2s; }
  .slider::before{ content:''; position:absolute; width:18px; height:18px; left:3px; top:3px; background:#fff; border-radius:50%; transition:0.2s; box-shadow:0 1px 2px rgba(0,0,0,0.2); }
  .switch input:checked + .slider{ background:var(--money); }
  .switch input:checked + .slider::before{ transform:translateX(18px); }

  .seg{ display:flex; gap:10px; flex-wrap:wrap; }
  .seg-opt{ flex:1; min-width:140px; border:1.5px solid var(--line); border-radius:9px; padding:14px; cursor:pointer; transition:border-color 0.15s, background 0.15s; position:relative; }
  .seg-opt input{ position:absolute; opacity:0; }
  .seg-opt .swatch{ height:34px; border-radius:5px; margin-bottom:10px; }
  .seg-opt .name{ font-size:0.84rem; font-weight:600; }
  .seg-opt .desc{ font-size:0.72rem; color:var(--ink-soft); margin-top:2px; }
  .seg-opt.checked{ border-color:var(--money); box-shadow:0 0 0 3px rgba(63,122,92,0.12); }
  .seg-opt.checked::after{ content:'✓'; position:absolute; top:10px; right:12px; width:18px; height:18px; border-radius:50%; background:var(--money); color:#fff; font-size:0.62rem; display:flex; align-items:center; justify-content:center; }

  .connect-card{
    display:flex; align-items:center; gap:18px; padding:20px; border:1px solid var(--line); border-radius:10px;
    background:linear-gradient(135deg, rgba(63,122,92,0.05), rgba(201,161,90,0.05));
  }
  .connect-ico{ width:52px; height:52px; border-radius:12px; background:#fff; border:1px solid var(--line); display:flex; align-items:center; justify-content:center; flex:none; box-shadow:0 2px 6px rgba(0,0,0,0.05); }
  .connect-ico svg{ width:26px; height:26px; }
  .connect-body{ flex:1; min-width:180px; }
  .connect-body .t-label{ font-size:0.95rem; font-weight:600; display:flex; align-items:center; gap:8px; }
  .connect-body .t-sub{ font-size:0.8rem; color:var(--ink-soft); margin-top:3px; }
  .status-pill{ font-family:'IBM Plex Mono', monospace; font-size:0.64rem; letter-spacing:0.05em; text-transform:uppercase; padding:3px 9px; border-radius:20px; font-weight:600; }
  .status-pill.connected{ background:rgba(63,122,92,0.14); color:var(--money-deep); }
  .status-pill.off{ background:rgba(28,38,32,0.08); color:var(--ink-soft); }

  .mini-stats{ display:grid; grid-template-columns:repeat(3,1fr); gap:1px; background:var(--line); border:1px solid var(--line); border-radius:8px; overflow:hidden; margin-top:16px; }
  .mini-stat{ background:#fff; padding:14px 16px; }
  .mini-stat .n{ font-family:'Fraunces', serif; font-weight:600; font-size:1.3rem; color:var(--money-deep); }
  .mini-stat .l{ font-family:'IBM Plex Mono', monospace; font-size:0.62rem; letter-spacing:0.05em; text-transform:uppercase; color:var(--ink-soft); margin-top:3px; }

  .cat-list{ display:flex; flex-direction:column; }
  .cat-row{ display:flex; align-items:center; gap:12px; padding:12px 0; border-bottom:1px solid var(--line); }
  .cat-row:last-child{ border-bottom:none; }
  .cat-dot{ width:12px; height:12px; border-radius:4px; flex:none; }
  .cat-name{ flex:1; font-size:0.9rem; font-weight:600; }
  .cat-count{ font-family:'IBM Plex Mono', monospace; font-size:0.72rem; color:var(--ink-soft); }
  .cat-actions{ display:flex; gap:6px; }
  .icon-btn{ width:28px; height:28px; border-radius:6px; border:1px solid var(--line); background:#fff; color:var(--ink-soft); font-size:0.76rem; cursor:pointer; display:flex; align-items:center; justify-content:center; }
  .icon-btn:hover{ background:var(--paper-dim); }
  .add-cat-row{ display:flex; gap:10px; margin-top:16px; }
  .add-cat-row input{ flex:1; }
  .color-picker{ display:flex; gap:7px; align-items:center; }
  .color-dot{ width:22px; height:22px; border-radius:50%; cursor:pointer; border:2px solid transparent; }
  .color-dot.picked{ border-color:var(--ink); }

  .danger-panel{ border-color:rgba(177,70,47,0.3); }
  .danger-panel .panel-head h3{ color:var(--stamp); }
  .danger-row{ display:flex; align-items:center; justify-content:space-between; gap:16px; padding:14px 0; border-bottom:1px dashed rgba(177,70,47,0.25); }
  .danger-row:last-child{ border-bottom:none; }
  .danger-row .t-label{ font-size:0.9rem; font-weight:600; }
  .danger-row .t-sub{ font-size:0.78rem; color:var(--ink-soft); margin-top:2px; }
  .btn-danger{ background:#fff; border:1px solid var(--stamp); color:var(--stamp); padding:9px 16px; border-radius:6px; font-size:0.82rem; font-weight:600; cursor:pointer; white-space:nowrap; }
  .btn-danger:hover{ background:rgba(177,70,47,0.06); }

  .pw-link{ display:flex; align-items:center; justify-content:space-between; padding:6px 0; }
  .pw-link .t-label{ font-size:0.92rem; font-weight:600; }
  .pw-link .t-sub{ font-size:0.78rem; color:var(--ink-soft); margin-top:2px; }
  .pw-link a{ font-size:0.84rem; color:var(--money-deep); font-weight:600; border-bottom:1px solid var(--money-deep); }

  .session-row{ display:flex; align-items:center; justify-content:space-between; padding:12px 0; border-bottom:1px solid var(--line); gap:12px; }
  .session-row:last-child{ border-bottom:none; }
  .session-row .t-label{ font-size:0.88rem; font-weight:600; display:flex; align-items:center; gap:8px; }
  .session-row .t-sub{ font-size:0.76rem; color:var(--ink-soft); margin-top:2px; }
  .badge-now{ font-family:'IBM Plex Mono', monospace; font-size:0.6rem; letter-spacing:0.05em; text-transform:uppercase; background:rgba(63,122,92,0.14); color:var(--money-deep); padding:2px 7px; border-radius:20px; }

  @media (max-width: 900px){
    .settings-shell{ grid-template-columns:1fr; }
    .side-nav{ position:static; display:flex; overflow-x:auto; gap:4px; padding:8px; }
    .side-nav a{ white-space:nowrap; }
    .side-nav .danger-link{ border-top:none; border-left:1px dashed var(--line); }
    .field-row, .field-row3{ grid-template-columns:1fr; }
    .mini-stats{ grid-template-columns:1fr; }
  }
</style>
  <link rel="icon" type="image/png" href="data:image/png;base64,">
</head>
<body>

<nav>
  <div class="nav-inner">
    <a href="dashboard.html" class="brand">
      <span class="mark"><img src="data:image/png;base64," alt="TrackMySpend logo"></span>
      TrackMySpend
    </a>
    <div class="nav-links">
      <a href="dashboard.html">Dashboard</a>
      <a href="transactions.html">Transactions</a>
      <a href="budgets.html">Budgets</a>
      <a href="goals.html">Goals</a>
      <a href="reports.html">Reports</a>
      <a href="settings.html" class="active">Settings</a>
      <a href="profile.html">Profile</a>
    </div>
    <div class="nav-user">
      <span class="nav-avatar">RS</span>
      <a href="index.html" class="nav-signout">Sign out</a>
    </div>
  </div>
</nav>

<div class="wrap">

  <div class="page-head">
    <div class="kicker">Account · System</div>
    <h1>Settings</h1>
    <p>Control how TrackMySpend works for you — general preferences, notifications, connected accounts, categories, and your data.</p>
  </div>

  <div class="settings-shell">

    <nav class="side-nav" id="sideNav" aria-label="Settings sections">
      <a href="#general" class="active"><span class="ico">⚙</span> General</a>
      <a href="#notifications"><span class="ico">🔔</span> Notifications</a>
      <a href="#connected"><span class="ico">✉</span> Connected accounts</a>
      <a href="#categories"><span class="ico">▤</span> Categories</a>
      <a href="#appearance"><span class="ico">◑</span> Appearance</a>
      <a href="#security"><span class="ico">⛨</span> Security</a>
      <a href="#privacy"><span class="ico">⤓</span> Data &amp; privacy</a>
      <a href="#danger" class="danger-link"><span class="ico">⚠</span> Danger zone</a>
    </nav>

    <div class="settings-main">

      <section id="general">
        <div class="panel">
          <div class="panel-head">
            <div>
              <h3>General</h3>
              <div class="sub">Currency, language, and how dates &amp; numbers are shown.</div>
            </div>
            <span class="hint">Applies app-wide</span>
          </div>
          <form id="generalForm">
            <div class="field-row">
              <div class="field">
                <label for="currency">Preferred currency</label>
                <select id="currency">
                  <option selected>₹ Indian Rupee (INR)</option>
                  <option>$ US Dollar (USD)</option>
                  <option>€ Euro (EUR)</option>
                  <option>£ British Pound (GBP)</option>
                </select>
              </div>
              <div class="field">
                <label for="language">Language</label>
                <select id="language">
                  <option selected>English</option>
                  <option>हिन्दी (Hindi)</option>
                  <option>मराठी (Marathi)</option>
                </select>
              </div>
            </div>
            <div class="field-row3">
              <div class="field">
                <label for="dateFormat">Date format</label>
                <select id="dateFormat">
                  <option selected>DD MMM YYYY</option>
                  <option>MM/DD/YYYY</option>
                  <option>DD/MM/YYYY</option>
                </select>
              </div>
              <div class="field">
                <label for="weekStart">Week starts on</label>
                <select id="weekStart">
                  <option>Sunday</option>
                  <option selected>Monday</option>
                </select>
              </div>
              <div class="field">
                <label for="timezone">Timezone</label>
                <select id="timezone">
                  <option selected>IST (UTC+5:30)</option>
                  <option>UTC</option>
                </select>
              </div>
            </div>
            <div class="save-row">
              <span class="save-note" id="generalNote"><span class="dot"></span>Preferences saved</span>
              <button type="button" class="btn btn-ghost" onclick="document.getElementById('generalForm').reset()">Cancel</button>
              <button type="submit" class="btn btn-primary">Save changes</button>
            </div>
          </form>
        </div>
      </section>

      <section id="notifications">
        <div class="panel">
          <div class="panel-head">
            <div>
              <h3>Notifications</h3>
              <div class="sub">Choose what TrackMySpend should tell you, and when.</div>
            </div>
          </div>
          <div class="toggle-row">
            <div><div class="t-label">Budget alerts</div><div class="t-sub">Notify me when I'm close to a category limit</div></div>
            <label class="switch"><input type="checkbox" checked><span class="slider"></span></label>
          </div>
          <div class="toggle-row">
            <div><div class="t-label">Weekly summary</div><div class="t-sub">Email a spending recap every Monday</div></div>
            <label class="switch"><input type="checkbox" checked><span class="slider"></span></label>
          </div>
          <div class="toggle-row">
            <div><div class="t-label">Goal reminders</div><div class="t-sub">Nudge me if a savings goal hasn't moved in a week</div></div>
            <label class="switch"><input type="checkbox"><span class="slider"></span></label>
          </div>
          <div class="toggle-row">
            <div><div class="t-label">Gmail import ready</div><div class="t-sub">Tell me when a new batch of transactions is scanned in</div></div>
            <label class="switch"><input type="checkbox" checked><span class="slider"></span></label>
          </div>
          <div class="toggle-row">
            <div><div class="t-label">Low balance warning</div><div class="t-sub">Alert me if projected balance drops below ₹1,000</div></div>
            <label class="switch"><input type="checkbox" checked><span class="slider"></span></label>
          </div>
          <div class="toggle-row">
            <div><div class="t-label">Product updates</div><div class="t-sub">Occasional notes about new features</div></div>
            <label class="switch"><input type="checkbox"><span class="slider"></span></label>
          </div>
        </div>
      </section>

      <section id="connected">
        <div class="panel">
          <div class="panel-head">
            <div>
              <h3>Connected accounts</h3>
              <div class="sub">Link Gmail to auto-import transactions from bank &amp; UPI emails.</div>
            </div>
          </div>

          <div class="connect-card">
            <div class="connect-ico">
              <svg viewBox="0 0 24 24"><path fill="#EA4335" d="M12 12.7 2 5.5V19c0 .6.4 1 1 1h3V11l6 4.4 6-4.4v9h3c.6 0 1-.4 1-1V5.5z"/><path fill="#34A853" d="M6 20v-9l6 4.4V20z"/><path fill="#4285F4" d="M18 20v-9l-6 4.4V20z"/><path fill="#FBBC05" d="M2 5.5 12 12.7 22 5.5c0-.6-.5-1.5-1-1.5H3c-.5 0-1 .9-1 1.5z"/></svg>
            </div>
            <div class="connect-body">
              <div class="t-label">Gmail <span class="status-pill connected" id="gmailStatusPill">Connected</span></div>
              <div class="t-sub" id="gmailStatusSub">riya.sharma@gmail.com · last scanned 2 hours ago</div>
            </div>
            <a href="gmail-import.html" class="btn btn-primary btn-sm">Open import</a>
          </div>

          <div class="mini-stats">
            <div class="mini-stat"><div class="n">312</div><div class="l">Transactions imported</div></div>
            <div class="mini-stat"><div class="n">94%</div><div class="l">Auto-categorized</div></div>
            <div class="mini-stat"><div class="n">Every 6h</div><div class="l">Sync frequency</div></div>
          </div>

          <div class="field" style="margin-top:22px;">
            <label for="syncFreq">Auto-scan frequency</label>
            <select id="syncFreq">
              <option>Every hour</option>
              <option selected>Every 6 hours</option>
              <option>Once a day</option>
              <option>Manual only</option>
            </select>
          </div>

          <div class="toggle-row" style="border-top:1px solid var(--line); margin-top:6px; padding-top:14px;">
            <div><div class="t-label">Only scan bank &amp; UPI senders</div><div class="t-sub">Restrict scanning to a verified allow-list of senders (recommended)</div></div>
            <label class="switch"><input type="checkbox" checked><span class="slider"></span></label>
          </div>
          <div class="toggle-row">
            <div><div class="t-label">Auto-import high-confidence matches</div><div class="t-sub">Skip manual review when parsing confidence is above 90%</div></div>
            <label class="switch"><input type="checkbox"><span class="slider"></span></label>
          </div>

          <div class="save-row" style="margin-top:10px;">
            <button class="btn btn-ghost" id="disconnectGmail" type="button">Disconnect Gmail</button>
          </div>
        </div>
      </section>

      <section id="categories">
        <div class="panel">
          <div class="panel-head">
            <div>
              <h3>Categories</h3>
              <div class="sub">Used across manual entries and Gmail auto-import.</div>
            </div>
            <span class="hint">8 categories</span>
          </div>
          <div class="cat-list" id="catList">
            <div class="cat-row"><span class="cat-dot" style="background:#3f7a5c"></span><span class="cat-name">Food &amp; dining</span><span class="cat-count">86 entries</span><div class="cat-actions"><button class="icon-btn" title="Edit">✎</button><button class="icon-btn" title="Delete">✕</button></div></div>
            <div class="cat-row"><span class="cat-dot" style="background:#c9a15a"></span><span class="cat-name">Bills &amp; utilities</span><span class="cat-count">34 entries</span><div class="cat-actions"><button class="icon-btn" title="Edit">✎</button><button class="icon-btn" title="Delete">✕</button></div></div>
            <div class="cat-row"><span class="cat-dot" style="background:#b1462f"></span><span class="cat-name">Travel</span><span class="cat-count">21 entries</span><div class="cat-actions"><button class="icon-btn" title="Edit">✎</button><button class="icon-btn" title="Delete">✕</button></div></div>
            <div class="cat-row"><span class="cat-dot" style="background:#5b7fbd"></span><span class="cat-name">Savings &amp; goals</span><span class="cat-count">12 entries</span><div class="cat-actions"><button class="icon-btn" title="Edit">✎</button><button class="icon-btn" title="Delete">✕</button></div></div>
            <div class="cat-row"><span class="cat-dot" style="background:#8a6fbd"></span><span class="cat-name">Income</span><span class="cat-count">19 entries</span><div class="cat-actions"><button class="icon-btn" title="Edit">✎</button><button class="icon-btn" title="Delete">✕</button></div></div>
          </div>
          <div class="add-cat-row">
            <div class="color-picker" id="colorPicker">
              <span class="color-dot picked" style="background:#3f7a5c" data-c="#3f7a5c"></span>
              <span class="color-dot" style="background:#c9a15a" data-c="#c9a15a"></span>
              <span class="color-dot" style="background:#b1462f" data-c="#b1462f"></span>
              <span class="color-dot" style="background:#5b7fbd" data-c="#5b7fbd"></span>
              <span class="color-dot" style="background:#8a6fbd" data-c="#8a6fbd"></span>
            </div>
            <input type="text" id="newCatName" placeholder="New category name…">
            <button class="btn btn-primary btn-sm" id="addCatBtn" type="button">Add</button>
          </div>
        </div>
      </section>

      <section id="appearance">
        <div class="panel">
          <div class="panel-head">
            <div>
              <h3>Appearance</h3>
              <div class="sub">The passbook theme, your way.</div>
            </div>
          </div>
          <div class="seg" id="themeSeg">
            <label class="seg-opt checked">
              <input type="radio" name="theme" checked>
              <div class="swatch" style="background:linear-gradient(135deg,#101c16,#f4efe1 50%)"></div>
              <div class="name">Passbook (default)</div>
              <div class="desc">Dark cover, cream paper</div>
            </label>
            <label class="seg-opt">
              <input type="radio" name="theme">
              <div class="swatch" style="background:linear-gradient(135deg,#ece5d2,#fff 50%)"></div>
              <div class="name">Light paper</div>
              <div class="desc">All-cream, minimal contrast</div>
            </label>
            <label class="seg-opt">
              <input type="radio" name="theme">
              <div class="swatch" style="background:linear-gradient(135deg,#3d2b1f,#e8d9be 50%)"></div>
              <div class="name">Sepia ledger</div>
              <div class="desc">Warm tones, old paper feel</div>
            </label>
          </div>
          <div class="field" style="margin-top:20px; max-width:280px;">
            <label for="fontSize">Text size</label>
            <select id="fontSize">
              <option>Compact</option>
              <option selected>Comfortable</option>
              <option>Large</option>
            </select>
          </div>
        </div>
      </section>

      <section id="security">
        <div class="panel">
          <div class="panel-head"><h3>Security</h3></div>
          <div class="pw-link">
            <div><div class="t-label">Password</div><div class="t-sub">Last changed 2 months ago</div></div>
            <a href="change-password.html">Change password</a>
          </div>
        </div>
        <div class="panel" style="margin-top:24px;">
          <div class="panel-head">
            <div><h3>Two-factor authentication</h3><div class="sub">Add an extra step when logging in.</div></div>
          </div>
          <div class="toggle-row">
            <div><div class="t-label">Require OTP on login</div><div class="t-sub">Sent to your registered phone number</div></div>
            <label class="switch"><input type="checkbox" id="twoFA"><span class="slider"></span></label>
          </div>
        </div>
        <div class="panel" style="margin-top:24px;">
          <div class="panel-head"><h3>Active sessions</h3></div>
          <div class="session-row">
            <div><div class="t-label">Chrome · Windows <span class="badge-now">This device</span></div><div class="t-sub">Pune, India · active now</div></div>
          </div>
          <div class="session-row">
            <div><div class="t-label">Safari · iPhone</div><div class="t-sub">Pune, India · 3 days ago</div></div>
            <button class="btn btn-ghost btn-sm">Sign out</button>
          </div>
        </div>
      </section>

      <section id="privacy">
        <div class="panel">
          <div class="panel-head">
            <div><h3>Data &amp; privacy</h3><div class="sub">What TrackMySpend keeps, and for how long.</div></div>
          </div>
          <div class="toggle-row">
            <div><div class="t-label">Store raw Gmail snippets</div><div class="t-sub">Keep the original email text used to parse a transaction, for review</div></div>
            <label class="switch"><input type="checkbox" checked><span class="slider"></span></label>
          </div>
          <div class="toggle-row">
            <div><div class="t-label">Auto-delete imported email data</div><div class="t-sub">Remove raw snippets 30 days after import; parsed transactions stay</div></div>
            <label class="switch"><input type="checkbox"><span class="slider"></span></label>
          </div>
          <div class="danger-row" style="border-bottom:1px solid var(--line); border-style:solid;">
            <div><div class="t-label">Export your data</div><div class="t-sub">Download every transaction and category as a CSV</div></div>
            <button class="btn-danger" style="border-color:var(--line); color:var(--ink-soft);">Export CSV</button>
          </div>
        </div>
      </section>

      <section id="danger">
        <div class="panel danger-panel">
          <div class="panel-head"><h3>Danger zone</h3></div>
          <div class="danger-row">
            <div><div class="t-label">Revoke Gmail access</div><div class="t-sub">Stop all future scanning; previously imported entries remain</div></div>
            <button class="btn-danger">Revoke</button>
          </div>
          <div class="danger-row">
            <div><div class="t-label">Reset all categories</div><div class="t-sub">Restore the default category set; custom ones are removed</div></div>
            <button class="btn-danger">Reset</button>
          </div>
          <div class="danger-row">
            <div><div class="t-label">Delete account</div><div class="t-sub">Permanently erase your ledger. This can't be undone.</div></div>
            <button class="btn-danger">Delete account</button>
          </div>
        </div>
      </section>

    </div>
  </div>
</div>

<script>
  document.getElementById('generalForm').addEventListener('submit', (e) => {
    e.preventDefault();
    const note = document.getElementById('generalNote');
    note.classList.add('show');
    setTimeout(() => note.classList.remove('show'), 2600);
  });

  const links = Array.from(document.querySelectorAll('#sideNav a[href^="#"]'));
  const sections = links.map(l => document.querySelector(l.getAttribute('href')));
  const spy = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if(entry.isIntersecting){
        const id = '#' + entry.target.id;
        links.forEach(l => l.classList.toggle('active', l.getAttribute('href') === id && !l.classList.contains('danger-link')));
      }
    });
  }, { rootMargin: '-40% 0px -50% 0px' });
  sections.forEach(s => s && spy.observe(s));

  document.querySelectorAll('#themeSeg .seg-opt').forEach(opt => {
    opt.addEventListener('click', () => {
      document.querySelectorAll('#themeSeg .seg-opt').forEach(o => o.classList.remove('checked'));
      opt.classList.add('checked');
      opt.querySelector('input').checked = true;
    });
  });

  document.querySelectorAll('#colorPicker .color-dot').forEach(dot => {
    dot.addEventListener('click', () => {
      document.querySelectorAll('#colorPicker .color-dot').forEach(d => d.classList.remove('picked'));
      dot.classList.add('picked');
    });
  });

  document.getElementById('addCatBtn').addEventListener('click', () => {
    const nameInput = document.getElementById('newCatName');
    const name = nameInput.value.trim();
    if(!name) { nameInput.focus(); return; }
    const color = document.querySelector('#colorPicker .color-dot.picked').dataset.c;
    const row = document.createElement('div');
    row.className = 'cat-row';
    row.innerHTML = `<span class="cat-dot" style="background:${color}"></span><span class="cat-name">${name}</span><span class="cat-count">0 entries</span><div class="cat-actions"><button class="icon-btn" title="Edit">✎</button><button class="icon-btn" title="Delete">✕</button></div>`;
    document.getElementById('catList').appendChild(row);
    nameInput.value = '';
  });

  document.getElementById('catList').addEventListener('click', (e) => {
    if(e.target.title === 'Delete'){
      e.target.closest('.cat-row').remove();
    }
  });

  document.getElementById('disconnectGmail').addEventListener('click', function(){
    const pill = document.getElementById('gmailStatusPill');
    const sub = document.getElementById('gmailStatusSub');
    const connected = pill.classList.contains('connected');
    if(connected){
      pill.classList.remove('connected'); pill.classList.add('off');
      pill.textContent = 'Not connected';
      sub.textContent = 'Connect Gmail to auto-import transactions';
      this.textContent = 'Connect Gmail';
    } else {
      pill.classList.remove('off'); pill.classList.add('connected');
      pill.textContent = 'Connected';
      sub.textContent = 'riya.sharma@gmail.com · last scanned just now';
      this.textContent = 'Disconnect Gmail';
    }
  });
</script>

</body>
</html>
