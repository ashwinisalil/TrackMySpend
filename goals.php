<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Savings Goals — TrackMySpend</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,400;0,9..144,500;0,9..144,600;0,9..144,700;1,9..144,500&family=IBM+Plex+Mono:wght@400;500;600&family=IBM+Plex+Sans:wght@400;500;600&display=swap" rel="stylesheet">
<style>
  :root{
    --cover: #101c16;
    --cover-deep: #0a1410;
    --paper: #f4efe1;
    --paper-dim: #ece5d2;
    --ink: #1c2620;
    --ink-soft: #4b5850;
    --money: #3f7a5c;
    --money-deep: #2c5842;
    --stamp: #b1462f;
    --gold: #c9a15a;
    --line: rgba(28,38,32,0.14);
    --line-on-cover: rgba(244,239,225,0.16);
  }
  *{ margin:0; padding:0; box-sizing:border-box; }
  html,body{ min-height:100%; }
  body{
    background:var(--paper);
    color:var(--ink);
    font-family:'IBM Plex Sans', sans-serif;
    -webkit-font-smoothing:antialiased;
  }
  @media (prefers-reduced-motion: reduce){
    *{ animation-duration:0.001ms !important; animation-iteration-count:1 !important; transition-duration:0.001ms !important; }
  }
  a{ color:inherit; text-decoration:none; }
  .display{ font-family:'Fraunces', serif; }
  .mono{ font-family:'IBM Plex Mono', monospace; }
  ::selection{ background:var(--gold); color:var(--cover-deep); }
  .wrap{ max-width:1180px; margin:0 auto; padding:0 32px; }
  button{ font-family:inherit; cursor:pointer; }

  /* ---------------- NAV ---------------- */
  nav{ background:var(--cover); border-bottom:1px solid var(--line-on-cover); position:sticky; top:0; z-index:60; }
  .nav-inner{ display:flex; align-items:center; justify-content:space-between; padding:16px 32px; max-width:1180px; margin:0 auto; }
  .brand{ display:flex; align-items:center; gap:11px; font-family:'Fraunces', serif; font-weight:600; font-size:1.1rem; color:var(--paper); }
  .brand .mark{ width:32px; height:32px; border-radius:7px; overflow:hidden; flex:none; background:var(--paper); box-shadow:0 2px 0 var(--money-deep); }
  .brand .mark img{ width:100%; height:100%; object-fit:cover; display:block; }
  .nav-links{ display:flex; gap:20px; font-size:0.86rem; }
  .nav-links a{ position:relative; color:rgba(244,239,225,0.68); transition:color 0.2s; padding-bottom:4px; }
  .nav-links a::after{ content:''; position:absolute; left:0; right:0; bottom:0; height:2px; background:var(--gold); transform:scaleX(0); transform-origin:left; transition:transform 0.25s ease; }
  .nav-links a:hover{ color:var(--paper); }
  .nav-links a:hover::after{ transform:scaleX(1); }
  .nav-links a.active{ color:var(--paper); }
  .nav-links a.active::after{ transform:scaleX(1); }
  .nav-user{ display:flex; align-items:center; gap:10px; }
  .nav-avatar{ width:32px; height:32px; border-radius:50%; background:var(--money); color:var(--paper); display:flex; align-items:center; justify-content:center; font-family:'IBM Plex Mono', monospace; font-weight:600; font-size:0.78rem; border:1.5px solid var(--gold); }
  .nav-signout{ font-size:0.82rem; color:rgba(244,239,225,0.55); border:1px solid var(--line-on-cover); padding:7px 13px; border-radius:5px; transition:opacity 0.2s; }
  .nav-signout:hover{ opacity:0.7; }
  @media (max-width: 860px){ .nav-links{ display:none; } }

  /* ---------------- PAGE HEAD ---------------- */
  .page-head{ padding:44px 0 0; display:flex; align-items:flex-end; justify-content:space-between; gap:20px; flex-wrap:wrap; }
  .kicker{ font-family:'IBM Plex Mono', monospace; font-size:0.72rem; letter-spacing:0.1em; text-transform:uppercase; color:var(--money-deep); margin-bottom:8px; }
  .page-head h1{ font-family:'Fraunces', serif; font-weight:600; font-size:2rem; }
  .page-head p{ margin-top:8px; color:var(--ink-soft); font-size:0.96rem; max-width:520px; }
  .add-btn{
    flex:none; padding:12px 22px; border-radius:6px; border:none; background:var(--money-deep); color:var(--paper);
    font-weight:600; font-size:0.88rem; box-shadow:0 3px 0 #1e3f2f; transition:transform 0.15s ease, box-shadow 0.15s ease;
  }
  .add-btn:hover{ transform:translateY(-1px); box-shadow:0 4px 0 #1e3f2f; }
  .add-btn:active{ transform:translateY(1px); box-shadow:0 1px 0 #1e3f2f; }

  /* ---------------- SUBTABS ---------------- */
  .subtabs{ margin-top:26px; display:flex; gap:6px; background:#fff; border:1px solid var(--line); border-radius:8px; padding:5px; width:fit-content; }
  .subtabs a{ padding:9px 18px; border-radius:5px; font-size:0.86rem; font-weight:600; color:var(--ink-soft); transition:background 0.2s, color 0.2s; }
  .subtabs a.active{ background:var(--money-deep); color:var(--paper); }
  .subtabs a:not(.active):hover{ background:var(--paper-dim); color:var(--ink); }

  /* ---------------- STATS ---------------- */
  .stats-row{ margin-top:24px; display:grid; grid-template-columns:repeat(4,1fr); gap:1px; background:var(--line); border:1px solid var(--line); border-radius:10px; overflow:hidden; }
  .stat{ background:#fff; padding:20px 22px; transition:background 0.2s; }
  .stat:hover{ background:var(--paper-dim); }
  .stat .label{ font-family:'IBM Plex Mono', monospace; font-size:0.66rem; letter-spacing:0.06em; text-transform:uppercase; color:var(--ink-soft); }
  .stat .value{ font-family:'Fraunces', serif; font-weight:600; font-size:1.55rem; margin-top:6px; color:var(--ink); }
  .stat .sub{ font-size:0.76rem; color:var(--ink-soft); margin-top:4px; }
  .stat .sub.up{ color:var(--money-deep); }

  /* ---------------- GOAL CARDS ---------------- */
  .goals-grid{ margin-top:28px; display:grid; grid-template-columns:repeat(auto-fill, minmax(300px,1fr)); gap:20px; }
  .goal-card{
    background:#fff; border:1px solid var(--line); border-radius:12px; padding:26px; position:relative; overflow:hidden;
    transition:transform 0.18s ease, box-shadow 0.18s ease;
  }
  .goal-card:hover{ transform:translateY(-3px); box-shadow:0 10px 26px rgba(16,28,22,0.1); }
  .goal-card.done::before{
    content:'✓ Completed'; position:absolute; top:14px; right:-30px; background:var(--money-deep); color:var(--paper);
    font-family:'IBM Plex Mono', monospace; font-size:0.62rem; padding:4px 34px; transform:rotate(35deg); letter-spacing:0.05em;
  }
  .goal-top{ display:flex; align-items:center; gap:12px; margin-bottom:18px; }
  .goal-icon{ width:42px; height:42px; border-radius:10px; background:var(--paper-dim); display:flex; align-items:center; justify-content:center; font-size:1.3rem; flex:none; }
  .goal-title{ font-family:'Fraunces', serif; font-weight:600; font-size:1.08rem; }
  .goal-deadline{ font-size:0.76rem; color:var(--ink-soft); margin-top:2px; }

  .ring-wrap{ display:flex; align-items:center; gap:20px; }
  .ring{ flex:none; }
  .ring circle{ transition:stroke-dashoffset 0.6s ease; }
  .ring-pct{ font-family:'Fraunces', serif; font-size:1.05rem; font-weight:600; }
  .goal-figures .amt-current{ font-family:'IBM Plex Mono', monospace; font-weight:600; font-size:1rem; }
  .goal-figures .amt-target{ font-family:'IBM Plex Mono', monospace; font-size:0.8rem; color:var(--ink-soft); }
  .goal-figures .remain{ font-size:0.78rem; color:var(--ink-soft); margin-top:4px; }

  .milestones{ display:flex; gap:5px; margin-top:16px; }
  .ms{ flex:1; height:6px; border-radius:3px; background:var(--paper-dim); }
  .ms.hit{ background:var(--gold); }

  .goal-actions{ display:flex; gap:8px; margin-top:18px; }
  .goal-actions .btn{ flex:1; padding:9px 0; font-size:0.82rem; text-align:center; }
  .goal-icon-del{ position:absolute; top:16px; right:16px; opacity:0; transition:opacity 0.15s; color:var(--ink-soft); font-size:0.8rem; }
  .goal-card:hover .goal-icon-del{ opacity:1; }
  .goal-icon-del:hover{ color:var(--stamp); }

  .add-goal-card{
    border:1.5px dashed var(--line); border-radius:12px; display:flex; align-items:center; justify-content:center;
    flex-direction:column; gap:8px; color:var(--ink-soft); min-height:220px; transition:all 0.15s;
  }
  .add-goal-card:hover{ border-color:var(--money); color:var(--money-deep); background:rgba(63,122,92,0.04); }
  .add-goal-plus{ font-size:1.8rem; font-family:'Fraunces', serif; }

  /* ---------------- PANELS / TABLE ---------------- */
  .main-grid{ margin:32px 0 80px; display:grid; grid-template-columns:1.5fr 1fr; gap:24px; align-items:start; }
  .panel{ background:#fff; border:1px solid var(--line); border-radius:10px; padding:26px 28px; }
  .panel + .panel{ margin-top:24px; }
  .panel-head{ display:flex; align-items:center; justify-content:space-between; margin-bottom:20px; gap:12px; flex-wrap:wrap; }
  .panel-head h3{ font-family:'Fraunces', serif; font-weight:600; font-size:1.14rem; }
  .panel-head .hint{ font-family:'IBM Plex Mono', monospace; font-size:0.7rem; color:var(--ink-soft); }
  .empty-msg{ color:var(--ink-soft); font-size:0.86rem; padding:18px 0; text-align:center; }

  table{ width:100%; border-collapse:collapse; font-size:0.86rem; }
  thead th{
    text-align:left; font-family:'IBM Plex Mono', monospace; font-size:0.66rem; letter-spacing:0.06em; text-transform:uppercase;
    color:var(--ink-soft); padding:0 10px 10px; border-bottom:1px solid var(--line);
  }
  tbody td{ padding:12px 10px; border-bottom:1px solid var(--line); vertical-align:middle; }
  tbody tr{ transition:background 0.15s ease; }
  tbody tr:hover{ background:var(--paper-dim); }
  tbody tr:last-child td{ border-bottom:none; }
  .badge{ display:inline-block; padding:4px 10px; border-radius:20px; font-size:0.72rem; font-weight:600; background:var(--paper-dim); color:var(--ink-soft); }
  .amt{ font-family:'IBM Plex Mono', monospace; font-weight:600; text-align:right; color:var(--money-deep); }
  .tx-date{ color:var(--ink-soft); font-family:'IBM Plex Mono', monospace; font-size:0.78rem; }

  .insight-item{ display:flex; gap:12px; padding:13px 0; border-bottom:1px solid var(--line); }
  .insight-item:last-child{ border-bottom:none; }
  .insight-dot{ width:8px; height:8px; border-radius:50%; background:var(--money); margin-top:6px; flex:none; }
  .insight-text b{ display:block; font-size:0.88rem; }
  .insight-text span{ font-size:0.78rem; color:var(--ink-soft); }

  /* ---------------- FORM ---------------- */
  .field{ display:flex; flex-direction:column; gap:8px; margin-bottom:16px; }
  .field:last-of-type{ margin-bottom:0; }
  .field-row{ display:grid; grid-template-columns:1fr 1fr; gap:14px; }
  .field label{ font-family:'IBM Plex Mono', monospace; font-size:0.7rem; letter-spacing:0.06em; text-transform:uppercase; color:var(--ink-soft); }
  input[type="text"], input[type="number"], input[type="date"]{
    width:100%; padding:11px 13px; border-radius:6px; border:1px solid var(--line); background:#fff;
    font-family:'IBM Plex Sans', sans-serif; font-size:0.9rem; color:var(--ink); transition:border-color 0.15s ease, box-shadow 0.15s ease;
  }
  input:focus{ outline:none; border-color:var(--money); box-shadow:0 0 0 3px rgba(63,122,92,0.14); }
  .icon-pick{ display:flex; gap:8px; flex-wrap:wrap; }
  .icon-opt{ width:38px; height:38px; border-radius:7px; border:1.5px solid var(--line); background:#fff; display:flex; align-items:center; justify-content:center; font-size:1.05rem; transition:all 0.15s; }
  .icon-opt.selected{ border-color:var(--money); background:rgba(63,122,92,0.1); }
  .btn{ padding:11px 20px; border-radius:6px; font-weight:600; font-size:0.88rem; border:none; cursor:pointer; transition:transform 0.15s ease, box-shadow 0.15s ease; }
  .btn-primary{ background:var(--money-deep); color:var(--paper); box-shadow:0 3px 0 #1e3f2f; }
  .btn-primary:hover{ transform:translateY(-1px); box-shadow:0 4px 0 #1e3f2f; }
  .btn-primary:active{ transform:translateY(1px); box-shadow:0 1px 0 #1e3f2f; }
  .btn-ghost{ background:none; border:1px solid var(--line); color:var(--ink-soft); }
  .btn-ghost:hover{ background:var(--paper-dim); }

  /* ---------------- MODAL ---------------- */
  .modal-backdrop{ position:fixed; inset:0; background:rgba(16,28,22,0.55); backdrop-filter:blur(2px); display:none; align-items:center; justify-content:center; z-index:100; padding:20px; }
  .modal-backdrop.show{ display:flex; }
  .modal{ background:#fff; border-radius:12px; padding:30px 32px; width:100%; max-width:440px; box-shadow:0 20px 60px rgba(0,0,0,0.3); }
  .modal h3{ font-family:'Fraunces', serif; font-weight:600; font-size:1.2rem; margin-bottom:18px; }
  .modal-actions{ display:flex; justify-content:flex-end; gap:10px; margin-top:20px; }

  .toast{
    position:fixed; bottom:26px; right:26px; background:var(--cover); color:var(--paper); padding:13px 20px; border-radius:8px;
    font-size:0.86rem; box-shadow:0 8px 24px rgba(0,0,0,0.25); display:flex; align-items:center; gap:10px;
    transform:translateY(20px); opacity:0; pointer-events:none; transition:all 0.3s ease; z-index:200;
  }
  .toast.show{ transform:translateY(0); opacity:1; }
  .toast .dot{ width:7px; height:7px; border-radius:50%; background:var(--gold); }

  @media (max-width: 900px){
    .main-grid{ grid-template-columns:1fr; }
    .stats-row{ grid-template-columns:1fr 1fr; }
    .field-row{ grid-template-columns:1fr; }
  }
</style>
  <link rel="icon" type="image/png" href="data:image/png;base64,">
</head>
<body>

<nav>
  <div class="nav-inner">
    <a href="dashboard.html" class="brand">
      <span class="mark"><img src="data:image/png;base64," alt="TrackMySpend logo"></span>
      TrackMySpend
    </a>
    <div class="nav-links">
      <a href="dashboard.html">Dashboard</a>
      <a href="transactions.html">Transactions</a>
      <a href="budgets.html">Budgets</a>
      <a href="goals.html" class="active">Goals</a>
      <a href="reports.html">Reports</a>
      <a href="settings.html">Settings</a>
      <a href="profile.html">Profile</a>
    </div>
    <div class="nav-user">
      <span class="nav-avatar">RS</span>
      <a href="index.html" class="nav-signout">Sign out</a>
    </div>
  </div>
</nav>

<div class="wrap">

  <div class="page-head">
    <div>
      <div class="kicker">Money Management</div>
      <h1>Savings Goals</h1>
      <p>Set a target, a deadline, and watch your progress build — one contribution at a time.</p>
    </div>
    <button class="add-btn" id="openAddGoal">+ New goal</button>
  </div>

  <div class="subtabs">
    <a href="budgets.html">Budget Overview</a>
    <a href="goals.html" class="active">Savings Goals</a>
    <a href="categories.html">Manage Categories</a>
  </div>

  <div class="stats-row" id="statsRow"></div>

  <div class="goals-grid" id="goalsGrid"></div>

  <div class="main-grid">

    <div class="panel">
      <div class="panel-head">
        <h3>Contribution history</h3>
        <span class="hint">Most recent first</span>
      </div>
      <table>
        <thead>
          <tr>
            <th>Date</th>
            <th>Goal</th>
            <th style="text-align:right;">Amount</th>
          </tr>
        </thead>
        <tbody id="contribBody"></tbody>
      </table>
    </div>

    <div class="panel">
      <div class="panel-head">
        <h3>Insights</h3>
      </div>
      <div id="insightsPanel"></div>
    </div>

  </div>
</div>

<!-- ADD GOAL MODAL -->
<div class="modal-backdrop" id="goalModal">
  <div class="modal">
    <h3>New savings goal</h3>
    <form id="goalForm">
      <div class="field">
        <label>Goal name</label>
        <input type="text" id="gName" placeholder="e.g. New Laptop" required>
      </div>
      <div class="field">
        <label>Icon</label>
        <div class="icon-pick" id="iconPick">
          <button type="button" class="icon-opt selected" data-icon="💻">💻</button>
          <button type="button" class="icon-opt" data-icon="✈️">✈️</button>
          <button type="button" class="icon-opt" data-icon="🛡️">🛡️</button>
          <button type="button" class="icon-opt" data-icon="🏍️">🏍️</button>
          <button type="button" class="icon-opt" data-icon="🎓">🎓</button>
          <button type="button" class="icon-opt" data-icon="🏡">🏡</button>
          <button type="button" class="icon-opt" data-icon="💍">💍</button>
          <button type="button" class="icon-opt" data-icon="🎁">🎁</button>
        </div>
      </div>
      <div class="field-row">
        <div class="field">
          <label>Target amount (₹)</label>
          <input type="number" id="gTarget" min="0" step="1" required>
        </div>
        <div class="field">
          <label>Already saved (₹)</label>
          <input type="number" id="gCurrent" min="0" step="1" value="0">
        </div>
      </div>
      <div class="field">
        <label>Target date</label>
        <input type="date" id="gDeadline" required>
      </div>
      <div class="modal-actions">
        <button type="button" class="btn btn-ghost" id="cancelGoal">Cancel</button>
        <button type="submit" class="btn btn-primary">Create goal</button>
      </div>
    </form>
  </div>
</div>

<!-- ADD CONTRIBUTION MODAL -->
<div class="modal-backdrop" id="contribModal">
  <div class="modal">
    <h3>Add contribution</h3>
    <form id="contribForm">
      <input type="hidden" id="contribGoalId">
      <div class="field">
        <label id="contribGoalLabel">Adding to goal</label>
        <input type="number" id="contribAmount" min="1" step="1" placeholder="Amount in ₹" required>
      </div>
      <div class="modal-actions">
        <button type="button" class="btn btn-ghost" id="cancelContrib">Cancel</button>
        <button type="submit" class="btn btn-primary">Add funds</button>
      </div>
    </form>
  </div>
</div>

<div class="toast" id="toast"><span class="dot"></span><span id="toastMsg">Saved</span></div>

<script>
/* ============ STATE ============ */
const fmt = n => '₹' + Number(n).toLocaleString('en-IN', {maximumFractionDigits:0});

const seedGoals = [
  { id:'g1', name:'Emergency Fund', icon:'🛡️', target:50000, current:31500, deadline:'2026-12-31' },
  { id:'g2', name:'New Laptop',     icon:'💻', target:65000, current:22000, deadline:'2026-10-15' },
  { id:'g3', name:'Goa Trip',       icon:'✈️', target:20000, current:20000, deadline:'2026-09-01' },
];

const seedContribs = [
  { id:'c1', goalId:'g1', goal:'Emergency Fund', date:'2026-07-15', amount:3000 },
  { id:'c2', goalId:'g2', goal:'New Laptop',     date:'2026-07-10', amount:4000 },
  { id:'c3', goalId:'g3', goal:'Goa Trip',       date:'2026-07-05', amount:2000 },
  { id:'c4', goalId:'g1', goal:'Emergency Fund', date:'2026-06-28', amount:3000 },
  { id:'c5', goalId:'g2', goal:'New Laptop',     date:'2026-06-20', amount:4000 },
];

function load(key, fallback){
  const raw = localStorage.getItem(key);
  if(raw){ try { return JSON.parse(raw); } catch(e){ return fallback; } }
  localStorage.setItem(key, JSON.stringify(fallback));
  return fallback;
}
function save(key, val){ localStorage.setItem(key, JSON.stringify(val)); }

let goals = load('tms_goals', seedGoals);
let contribs = load('tms_contributions', seedContribs);

/* ============ RENDER: STATS ============ */
function renderStats(){
  const totalSaved = goals.reduce((s,g)=>s+g.current,0);
  const active = goals.filter(g=>g.current < g.target).length;
  const completed = goals.filter(g=>g.current >= g.target).length;
  const avgMonthly = contribs.length ? Math.round(contribs.reduce((s,c)=>s+c.amount,0) / 3) : 0;

  const stats = [
    { label:'Total saved', value:fmt(totalSaved), sub:goals.length+' goals total' },
    { label:'Active goals', value:active, sub:'In progress' },
    { label:'Completed', value:completed, sub:'Fully funded', cls:'up' },
    { label:'Avg. monthly contribution', value:fmt(avgMonthly), sub:'Last 3 months' },
  ];
  document.getElementById('statsRow').innerHTML = stats.map(s=>`
    <div class="stat">
      <div class="label">${s.label}</div>
      <div class="value">${s.value}</div>
      <div class="sub ${s.cls||''}">${s.sub}</div>
    </div>
  `).join('');
}

/* ============ RING SVG ============ */
function ringSvg(pct, id){
  const r = 34, c = 2*Math.PI*r;
  const offset = c - (Math.min(pct,100)/100)*c;
  const color = pct >= 100 ? 'var(--gold)' : 'var(--money)';
  return `
  <svg class="ring" width="84" height="84" viewBox="0 0 84 84">
    <circle cx="42" cy="42" r="${r}" fill="none" stroke="var(--paper-dim)" stroke-width="8"/>
    <circle cx="42" cy="42" r="${r}" fill="none" stroke="${color}" stroke-width="8"
      stroke-linecap="round" stroke-dasharray="${c}" stroke-dashoffset="${offset}"
      transform="rotate(-90 42 42)"/>
    <text x="42" y="47" text-anchor="middle" class="ring-pct" fill="var(--ink)" font-size="15">${Math.min(pct,100)}%</text>
  </svg>`;
}

/* ============ RENDER: GOALS GRID ============ */
function daysLeft(deadline){
  const d = Math.ceil((new Date(deadline) - new Date('2026-07-21')) / 86400000);
  return d;
}

function renderGoals(){
  const grid = document.getElementById('goalsGrid');
  const cards = goals.map(g=>{
    const pct = Math.round((g.current/g.target)*100);
    const done = g.current >= g.target;
    const remain = Math.max(g.target - g.current, 0);
    const dl = daysLeft(g.deadline);
    const milestones = [25,50,75,100].map(m => pct >= m ? 'hit' : '').map(cls=>`<div class="ms ${cls}"></div>`).join('');
    return `
    <div class="goal-card ${done?'done':''}">
      <span class="goal-icon-del" onclick="deleteGoal('${g.id}')" title="Delete goal">✕</span>
      <div class="goal-top">
        <div class="goal-icon">${g.icon}</div>
        <div>
          <div class="goal-title">${g.name}</div>
          <div class="goal-deadline">${done ? 'Goal reached 🎉' : (dl>=0 ? dl+' days left · '+g.deadline : 'Past due · '+g.deadline)}</div>
        </div>
      </div>
      <div class="ring-wrap">
        ${ringSvg(pct)}
        <div class="goal-figures">
          <div class="amt-current">${fmt(g.current)}</div>
          <div class="amt-target">of ${fmt(g.target)} target</div>
          <div class="remain">${done ? 'Fully funded' : fmt(remain)+' to go'}</div>
        </div>
      </div>
      <div class="milestones">${milestones}</div>
      <div class="goal-actions">
        ${done ? '' : `<button class="btn btn-primary" onclick="openContrib('${g.id}','${g.name}')">Add funds</button>`}
        <button class="btn btn-ghost" onclick="openEditGoal('${g.id}')">Edit</button>
      </div>
    </div>`;
  }).join('');

  grid.innerHTML = cards + `
    <div class="add-goal-card" id="addGoalCard">
      <div class="add-goal-plus">+</div>
      <div>Create a new goal</div>
    </div>`;
  document.getElementById('addGoalCard').addEventListener('click', ()=> document.getElementById('openAddGoal').click());
}

/* ============ RENDER: CONTRIBUTIONS TABLE ============ */
function renderContribs(){
  const body = document.getElementById('contribBody');
  const rows = [...contribs].sort((a,b)=> new Date(b.date) - new Date(a.date));
  if(!rows.length){ body.innerHTML = '<tr><td colspan="3" class="empty-msg">No contributions yet.</td></tr>'; return; }
  body.innerHTML = rows.map(c=>`
    <tr>
      <td class="tx-date">${c.date}</td>
      <td><span class="badge">${c.goal}</span></td>
      <td class="amt">+ ${fmt(c.amount)}</td>
    </tr>
  `).join('');
}

/* ============ RENDER: INSIGHTS ============ */
function renderInsights(){
  const panel = document.getElementById('insightsPanel');
  const active = goals.filter(g => g.current < g.target);
  if(!active.length){
    panel.innerHTML = `<div class="insight-item"><div class="insight-dot"></div><div class="insight-text"><b>All goals funded</b><span>Consider setting a new target to keep the momentum going.</span></div></div>`;
    return;
  }
  panel.innerHTML = active.map(g=>{
    const dl = Math.max(daysLeft(g.deadline), 1);
    const monthsLeft = Math.max(Math.ceil(dl/30), 1);
    const remain = g.target - g.current;
    const perMonth = Math.ceil(remain / monthsLeft);
    return `
    <div class="insight-item">
      <div class="insight-dot"></div>
      <div class="insight-text">
        <b>${g.name}</b>
        <span>Save about ${fmt(perMonth)} / month to hit your target by ${g.deadline}.</span>
      </div>
    </div>`;
  }).join('');
}

function renderAll(){ renderStats(); renderGoals(); renderContribs(); renderInsights(); }
renderAll();

/* ============ DELETE ============ */
function deleteGoal(id){
  goals = goals.filter(g=>g.id!==id);
  contribs = contribs.filter(c=>c.goalId!==id);
  save('tms_goals', goals);
  save('tms_contributions', contribs);
  renderAll();
  showToast('Goal removed');
}

/* ============ ADD GOAL MODAL ============ */
const goalModal = document.getElementById('goalModal');
const goalForm = document.getElementById('goalForm');
let selectedIcon = '💻';
let editingGoalId = null;

document.getElementById('openAddGoal').addEventListener('click', ()=>{
  editingGoalId = null;
  goalForm.reset();
  selectedIcon = '💻';
  setIconSelection(selectedIcon);
  goalModal.querySelector('h3').textContent = 'New savings goal';
  goalModal.classList.add('show');
});
document.getElementById('cancelGoal').addEventListener('click', ()=> goalModal.classList.remove('show'));
goalModal.addEventListener('click', e=>{ if(e.target === goalModal) goalModal.classList.remove('show'); });

document.querySelectorAll('#iconPick .icon-opt').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    selectedIcon = btn.dataset.icon;
    setIconSelection(selectedIcon);
  });
});
function setIconSelection(icon){
  document.querySelectorAll('#iconPick .icon-opt').forEach(b=>{
    b.classList.toggle('selected', b.dataset.icon === icon);
  });
}

function openEditGoal(id){
  const g = goals.find(x=>x.id===id);
  if(!g) return;
  editingGoalId = id;
  document.getElementById('gName').value = g.name;
  document.getElementById('gTarget').value = g.target;
  document.getElementById('gCurrent').value = g.current;
  document.getElementById('gDeadline').value = g.deadline;
  selectedIcon = g.icon;
  setIconSelection(selectedIcon);
  goalModal.querySelector('h3').textContent = 'Edit goal';
  goalModal.classList.add('show');
}

goalForm.addEventListener('submit', e=>{
  e.preventDefault();
  const name = document.getElementById('gName').value.trim();
  const target = Number(document.getElementById('gTarget').value);
  const current = Number(document.getElementById('gCurrent').value) || 0;
  const deadline = document.getElementById('gDeadline').value;
  if(!name || target <= 0 || !deadline) return;

  if(editingGoalId){
    const g = goals.find(x=>x.id===editingGoalId);
    g.name=name; g.icon=selectedIcon; g.target=target; g.current=current; g.deadline=deadline;
    showToast('Goal updated');
  } else {
    goals.push({ id:'g'+Date.now(), name, icon:selectedIcon, target, current, deadline });
    showToast('Goal created');
  }
  save('tms_goals', goals);
  goalModal.classList.remove('show');
  renderAll();
});

/* ============ ADD CONTRIBUTION MODAL ============ */
const contribModal = document.getElementById('contribModal');
const contribForm = document.getElementById('contribForm');

function openContrib(goalId, goalName){
  document.getElementById('contribGoalId').value = goalId;
  document.getElementById('contribGoalLabel').textContent = 'Adding to “' + goalName + '” (₹)';
  document.getElementById('contribAmount').value = '';
  contribModal.classList.add('show');
}
document.getElementById('cancelContrib').addEventListener('click', ()=> contribModal.classList.remove('show'));
contribModal.addEventListener('click', e=>{ if(e.target === contribModal) contribModal.classList.remove('show'); });

contribForm.addEventListener('submit', e=>{
  e.preventDefault();
  const goalId = document.getElementById('contribGoalId').value;
  const amount = Number(document.getElementById('contribAmount').value);
  const g = goals.find(x=>x.id===goalId);
  if(!g || amount <= 0) return;

  g.current = Math.min(g.current + amount, g.target);
  contribs.push({ id:'c'+Date.now(), goalId:g.id, goal:g.name, date:'2026-07-21', amount });
  save('tms_goals', goals);
  save('tms_contributions', contribs);
  contribModal.classList.remove('show');
  renderAll();
  showToast('Contribution added');
});

/* ============ TOAST ============ */
let toastTimer;
function showToast(msg){
  const toast = document.getElementById('toast');
  document.getElementById('toastMsg').textContent = msg;
  toast.classList.add('show');
  clearTimeout(toastTimer);
  toastTimer = setTimeout(()=> toast.classList.remove('show'), 2400);
}
</script>

</body>
</html>
