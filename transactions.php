<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Transactions — TrackMySpend</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,400;0,9..144,500;0,9..144,600;0,9..144,700;1,9..144,500&family=IBM+Plex+Mono:wght@400;500;600&family=IBM+Plex+Sans:wght@400;500;600&display=swap" rel="stylesheet">
<style>
  :root{
    --cover: #101c16;
    --cover-deep: #0a1410;
    --paper: #f4efe1;
    --paper-dim: #ece5d2;
    --ink: #1c2620;
    --ink-soft: #4b5850;
    --money: #3f7a5c;
    --money-deep: #2c5842;
    --stamp: #b1462f;
    --gold: #c9a15a;
    --line: rgba(28,38,32,0.14);
    --line-on-cover: rgba(244,239,225,0.16);
  }

  *{ margin:0; padding:0; box-sizing:border-box; }
  html,body{ min-height:100%; }
  body{
    background:var(--paper);
    color:var(--ink);
    font-family:'IBM Plex Sans', sans-serif;
    -webkit-font-smoothing:antialiased;
  }
  @media (prefers-reduced-motion: reduce){
    *{ animation-duration:0.001ms !important; animation-iteration-count:1 !important; transition-duration:0.001ms !important; }
  }
  a{ color:inherit; text-decoration:none; }
  .display{ font-family:'Fraunces', serif; }
  .mono{ font-family:'IBM Plex Mono', monospace; }
  ::selection{ background:var(--gold); color:var(--cover-deep); }

  .wrap{ max-width:1180px; margin:0 auto; padding:0 32px; }

  /* ---------------- NAV ---------------- */
  nav{ background:var(--cover); border-bottom:1px solid var(--line-on-cover); }
  .nav-inner{ display:flex; align-items:center; justify-content:space-between; padding:16px 32px; max-width:1180px; margin:0 auto; }
  .brand{ display:flex; align-items:center; gap:11px; font-family:'Fraunces', serif; font-weight:600; font-size:1.1rem; color:var(--paper); }
  .brand .mark{ width:32px; height:32px; border-radius:7px; overflow:hidden; flex:none; background:var(--paper); box-shadow:0 2px 0 var(--money-deep); display:flex; align-items:center; justify-content:center; }
  .brand .mark img{ width:100%; height:100%; object-fit:cover; display:block; }
  .nav-links{ display:flex; gap:20px; font-size:0.86rem; }
  .nav-links a{ position:relative; color:rgba(244,239,225,0.68); transition:color 0.2s; padding-bottom:4px; }
  .nav-links a::after{ content:''; position:absolute; left:0; right:0; bottom:0; height:2px; background:var(--gold); transform:scaleX(0); transform-origin:left; transition:transform 0.25s ease; }
  .nav-links a:hover{ color:var(--paper); }
  .nav-links a:hover::after{ transform:scaleX(1); }
  .nav-links a.active{ color:var(--paper); }
  .nav-links a.active::after{ transform:scaleX(1); }
  .nav-user{ display:flex; align-items:center; gap:10px; }
  .nav-avatar{ width:32px; height:32px; border-radius:50%; background:var(--money); color:var(--paper); display:flex; align-items:center; justify-content:center; font-family:'IBM Plex Mono', monospace; font-weight:600; font-size:0.78rem; border:1.5px solid var(--gold); }
  .nav-signout{ font-size:0.82rem; color:rgba(244,239,225,0.55); border:1px solid var(--line-on-cover); padding:7px 13px; border-radius:5px; transition:opacity 0.2s; }
  .nav-signout:hover{ opacity:0.7; }
  @media (max-width: 760px){ .nav-links{ display:none; } }

  /* ---------------- PAGE HEAD ---------------- */
  .page-head{ padding:44px 0 0; display:flex; align-items:flex-end; justify-content:space-between; gap:20px; flex-wrap:wrap; }
  .kicker{ font-family:'IBM Plex Mono', monospace; font-size:0.72rem; letter-spacing:0.1em; text-transform:uppercase; color:var(--money-deep); margin-bottom:8px; }
  .page-head h1{ font-family:'Fraunces', serif; font-weight:600; font-size:2rem; }
  .page-head p{ margin-top:8px; color:var(--ink-soft); font-size:0.96rem; }
  .head-actions{ display:flex; gap:10px; flex:none; }
  .btn{ padding:11px 20px; border-radius:6px; font-weight:600; font-size:0.88rem; border:none; cursor:pointer; transition:transform 0.15s ease, box-shadow 0.15s ease; white-space:nowrap; }
  .btn-primary{ background:var(--stamp); color:var(--paper); box-shadow:0 3px 0 #7c2f1e; }
  .btn-primary:hover{ transform:translateY(-1px); box-shadow:0 4px 0 #7c2f1e; }
  .btn-primary.money{ background:var(--money-deep); box-shadow:0 3px 0 #1e3f2f; }
  .btn-primary.money:hover{ box-shadow:0 4px 0 #1e3f2f; }
  .btn-ghost{ background:none; border:1px solid var(--line); color:var(--ink-soft); }
  .btn-ghost:hover{ background:var(--paper-dim); }

  /* ---------------- STATS ROW ---------------- */
  .stats-row{ margin-top:28px; display:grid; grid-template-columns:repeat(4,1fr); gap:1px; background:var(--line); border:1px solid var(--line); border-radius:10px; overflow:hidden; }
  @media (max-width: 760px){ .stats-row{ grid-template-columns:repeat(2,1fr); } }
  .stat{ background:#fff; padding:20px 22px; }
  .stat .label{ font-family:'IBM Plex Mono', monospace; font-size:0.66rem; letter-spacing:0.06em; text-transform:uppercase; color:var(--ink-soft); }
  .stat .value{ font-family:'Fraunces', serif; font-weight:600; font-size:1.5rem; margin-top:6px; }
  .stat .value.in{ color:var(--money-deep); }
  .stat .value.out{ color:var(--stamp); }

  /* ---------------- FILTER BAR ---------------- */
  .filter-bar{ margin-top:24px; background:#fff; border:1px solid var(--line); border-radius:10px; padding:18px 20px; display:flex; flex-wrap:wrap; gap:12px; align-items:center; }
  .search-field{ flex:1 1 220px; position:relative; }
  .search-field input{ width:100%; padding:10px 14px 10px 34px; border-radius:6px; border:1px solid var(--line); font-family:'IBM Plex Sans', sans-serif; font-size:0.88rem; background:var(--paper-dim); }
  .search-field .icon{ position:absolute; left:12px; top:50%; transform:translateY(-50%); color:var(--ink-soft); font-size:0.85rem; }
  select.filter-select{ padding:10px 12px; border-radius:6px; border:1px solid var(--line); font-family:'IBM Plex Sans', sans-serif; font-size:0.85rem; background:#fff; color:var(--ink); flex:none; }
  input:focus, select:focus{ outline:none; border-color:var(--money); box-shadow:0 0 0 3px rgba(63,122,92,0.14); }
  .filter-clear{ font-size:0.82rem; color:var(--ink-soft); border-bottom:1px solid var(--line); flex:none; cursor:pointer; background:none; border-top:none; border-left:none; border-right:none; padding:0 0 1px; }
  .filter-clear:hover{ color:var(--ink); }

  /* ---------------- TABLE ---------------- */
  .table-panel{ margin:20px 0 80px; background:#fff; border:1px solid var(--line); border-radius:10px; overflow:hidden; }
  .table-scroll{ overflow-x:auto; }
  table{ width:100%; border-collapse:collapse; min-width:760px; }
  thead th{ text-align:left; font-family:'IBM Plex Mono', monospace; font-size:0.68rem; letter-spacing:0.06em; text-transform:uppercase; color:var(--ink-soft); padding:14px 18px; border-bottom:1px solid var(--line); background:var(--paper-dim); white-space:nowrap; cursor:pointer; user-select:none; }
  thead th .sort-arrow{ margin-left:4px; opacity:0.5; font-size:0.7rem; }
  thead th.active .sort-arrow{ opacity:1; color:var(--money-deep); }
  tbody td{ padding:14px 18px; border-bottom:1px solid var(--line); font-size:0.88rem; vertical-align:middle; }
  tbody tr:last-child td{ border-bottom:none; }
  tbody tr:hover{ background:rgba(63,122,92,0.04); }

  .cat-cell{ display:flex; align-items:center; gap:10px; }
  .cat-icon{ width:30px; height:30px; border-radius:7px; background:var(--paper-dim); display:flex; align-items:center; justify-content:center; font-size:0.85rem; flex:none; }
  .cat-name{ font-weight:600; }
  .note-sub{ color:var(--ink-soft); font-size:0.78rem; margin-top:2px; max-width:220px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }

  .type-badge{ font-family:'IBM Plex Mono', monospace; font-size:0.68rem; letter-spacing:0.04em; text-transform:uppercase; padding:4px 9px; border-radius:4px; display:inline-block; }
  .type-badge.expense{ background:#fbe6e0; color:var(--stamp); }
  .type-badge.income{ background:#dcece2; color:var(--money-deep); }

  .amount-cell{ font-family:'IBM Plex Mono', monospace; font-weight:600; text-align:right; white-space:nowrap; }
  .amount-cell.out{ color:var(--stamp); }
  .amount-cell.in{ color:var(--money-deep); }

  .row-actions{ display:flex; gap:6px; justify-content:flex-end; }
  .icon-btn{ width:28px; height:28px; border-radius:5px; border:1px solid var(--line); background:#fff; color:var(--ink-soft); font-size:0.8rem; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:background 0.15s; }
  .icon-btn:hover{ background:var(--paper-dim); }
  .icon-btn.danger:hover{ background:#fbe6e0; color:var(--stamp); border-color:var(--stamp); }

  .empty-state{ padding:64px 24px; text-align:center; }
  .empty-state .glyph{ font-size:2rem; margin-bottom:12px; }
  .empty-state h4{ font-family:'Fraunces', serif; font-weight:600; font-size:1.15rem; margin-bottom:6px; }
  .empty-state p{ color:var(--ink-soft); font-size:0.88rem; margin-bottom:18px; }

  .table-footer{ display:flex; align-items:center; justify-content:space-between; padding:14px 18px; border-top:1px solid var(--line); font-family:'IBM Plex Mono', monospace; font-size:0.76rem; color:var(--ink-soft); flex-wrap:wrap; gap:10px; }

  /* ---------------- MODAL ---------------- */
  .modal-overlay{ position:fixed; inset:0; background:rgba(16,28,22,0.55); display:none; align-items:center; justify-content:center; z-index:100; padding:20px; }
  .modal-overlay.show{ display:flex; }
  .modal-card{ background:#fff; border-radius:10px; padding:26px 26px 22px; max-width:360px; width:100%; box-shadow:0 20px 50px rgba(0,0,0,0.25); }
  .modal-card h4{ font-family:'Fraunces', serif; font-weight:600; font-size:1.1rem; margin-bottom:8px; }
  .modal-card p{ font-size:0.88rem; color:var(--ink-soft); line-height:1.5; }
  .modal-actions{ display:flex; justify-content:flex-end; gap:10px; margin-top:20px; }
  .btn-danger{ background:var(--stamp); color:var(--paper); }
  .btn-danger:hover{ opacity:0.92; }

  .toast{ position:fixed; bottom:24px; left:50%; transform:translateX(-50%) translateY(20px); background:var(--cover); color:var(--paper); padding:12px 20px; border-radius:8px; font-size:0.86rem; display:flex; align-items:center; gap:8px; opacity:0; pointer-events:none; transition:opacity 0.25s ease, transform 0.25s ease; z-index:200; }
  .toast.show{ opacity:1; transform:translateX(-50%) translateY(0); }
  .toast .dot{ width:6px; height:6px; border-radius:50%; background:var(--money); }
</style>
  <link rel="icon" type="image/png" href="data:image/png;base64,">
</head>
<body>

<nav>
  <div class="nav-inner">
    <a href="dashboard.html" class="brand">
      <div class="mark"><img src="data:image/png;base64," alt="TrackMySpend logo"></div>
      TrackMySpend
    </a>
    <div class="nav-links">
      <a href="dashboard.html">Dashboard</a>
      <a href="transactions.html" class="active">Transactions</a>
      <a href="budgets.html">Budgets</a>
      <a href="goals.html">Goals</a>
      <a href="reports.html">Reports</a>
      <a href="settings.html">Settings</a>
      <a href="profile.html">Profile</a>
    </div>
    <div class="nav-user">
      <span class="nav-avatar">RS</span>
      <a href="index.html" class="nav-signout">Sign out</a>
    </div>
  </div>
</nav>

<div class="wrap">

  <div class="page-head">
    <div>
      <div class="kicker">Transaction history</div>
      <h1>Every entry, in one place</h1>
      <p>Search, filter, and sort everything you've logged.</p>
    </div>
    <div class="head-actions">
      <a href="expense.html" class="btn btn-primary">+ Add expense</a>
    </div>
  </div>

  <div class="stats-row">
    <div class="stat">
      <div class="label">Total transactions</div>
      <div class="value" id="statCount">0</div>
    </div>
    <div class="stat">
      <div class="label">Total in</div>
      <div class="value in" id="statIn">₹0</div>
    </div>
    <div class="stat">
      <div class="label">Total out</div>
      <div class="value out" id="statOut">₹0</div>
    </div>
    <div class="stat">
      <div class="label">Net</div>
      <div class="value" id="statNet">₹0</div>
    </div>
  </div>

  <div class="filter-bar">
    <div class="search-field">
      <span class="icon">🔍</span>
      <input type="text" id="searchInput" placeholder="Search by category or note…">
    </div>
    <select class="filter-select" id="typeFilter">
      <option value="all">All types</option>
      <option value="expense">Expense</option>
      <option value="income">Income</option>
    </select>
    <select class="filter-select" id="categoryFilter">
      <option value="all">All categories</option>
    </select>
    <select class="filter-select" id="dateFilter">
      <option value="all">All time</option>
      <option value="7">Last 7 days</option>
      <option value="30">Last 30 days</option>
      <option value="month">This month</option>
    </select>
    <select class="filter-select" id="sortSelect">
      <option value="date-desc">Newest first</option>
      <option value="date-asc">Oldest first</option>
      <option value="amount-desc">Amount: high to low</option>
      <option value="amount-asc">Amount: low to high</option>
    </select>
    <button type="button" class="filter-clear" id="clearFilters">Clear filters</button>
  </div>

  <div class="table-panel">
    <div class="table-scroll">
      <table>
        <thead>
          <tr>
            <th data-sort="date">Date <span class="sort-arrow">▾</span></th>
            <th data-sort="category">Category</th>
            <th>Payment method</th>
            <th>Type</th>
            <th data-sort="amount" style="text-align:right;">Amount <span class="sort-arrow">▾</span></th>
            <th style="text-align:right;">Actions</th>
          </tr>
        </thead>
        <tbody id="txBody"></tbody>
      </table>
    </div>
    <div id="emptyState" class="empty-state" style="display:none;">
      <div class="glyph">📭</div>
      <h4>No transactions match this view</h4>
      <p>Try clearing your filters, or log your first entry.</p>
      <a href="expense.html" class="btn btn-primary money" style="display:inline-block;">+ Add expense</a>
    </div>
    <div class="table-footer">
      <span id="resultCount">Showing 0 of 0</span>
      <span>Data stored locally on this device</span>
    </div>
  </div>
</div>

<!-- Delete confirmation modal -->
<div class="modal-overlay" id="deleteModal">
  <div class="modal-card">
    <h4>Delete this transaction?</h4>
    <p>This removes it from your history for good. This can't be undone.</p>
    <div class="modal-actions">
      <button type="button" class="btn btn-ghost" id="cancelDelete">Cancel</button>
      <button type="button" class="btn btn-danger" id="confirmDelete">Delete</button>
    </div>
  </div>
</div>

<div class="toast" id="toast"><span class="dot"></span><span id="toastText">Deleted</span></div>

<script>
  // ---------- data layer (same localStorage store used by expense.html) ----------
  const EXPENSES_KEY = 'trackmyspend_expenses';
  const INCOME_KEY = 'trackmyspend_income';
  const CATEGORIES_KEY = 'trackmyspend_categories';

  const CATEGORY_ICONS = {
    'Groceries': '🛒', 'Food & Dining': '🍽️', 'Transport': '🚌', 'Bills & Utilities': '💡',
    'Shopping': '🛍️', 'Entertainment': '🎬', 'Health': '💊', 'Rent': '🏠', 'Other': '📦',
    'Salary': '💼', 'Freelance': '🧾', 'Investment': '📈', 'Gift': '🎁'
  };

  function loadExpenses(){ return JSON.parse(localStorage.getItem(EXPENSES_KEY) || '[]'); }
  function saveExpenses(list){ localStorage.setItem(EXPENSES_KEY, JSON.stringify(list)); }
  function loadIncome(){ return JSON.parse(localStorage.getItem(INCOME_KEY) || '[]'); }
  function saveIncome(list){ localStorage.setItem(INCOME_KEY, JSON.stringify(list)); }

  function loadAllTransactions(){
    const expenses = loadExpenses().map(e => ({ ...e, type: 'expense' }));
    const income = loadIncome().map(i => ({ ...i, type: 'income' }));
    return expenses.concat(income);
  }

  function formatINR(n){ return '₹' + Number(n).toLocaleString('en-IN', { maximumFractionDigits: 2 }); }

  // ---------- populate category filter from whatever categories exist ----------
  function populateCategoryFilter(){
    const stored = JSON.parse(localStorage.getItem(CATEGORIES_KEY) || '[]');
    const fromTx = loadAllTransactions().map(t => t.category).filter(Boolean);
    const all = Array.from(new Set([...stored, ...fromTx])).sort();
    const sel = document.getElementById('categoryFilter');
    sel.innerHTML = '<option value="all">All categories</option>' +
      all.map(c => `<option value="${c}">${c}</option>`).join('');
  }

  // ---------- state ----------
  let sortState = { key: 'date', dir: 'desc' };
  let pendingDeleteId = null;
  let pendingDeleteType = null;

  const els = {
    search: document.getElementById('searchInput'),
    type: document.getElementById('typeFilter'),
    category: document.getElementById('categoryFilter'),
    date: document.getElementById('dateFilter'),
    sort: document.getElementById('sortSelect'),
    body: document.getElementById('txBody'),
    empty: document.getElementById('emptyState'),
    resultCount: document.getElementById('resultCount')
  };

  function withinDateFilter(dateStr, mode){
    if(mode === 'all') return true;
    const d = new Date(dateStr);
    const now = new Date();
    if(mode === 'month'){
      return d.getFullYear() === now.getFullYear() && d.getMonth() === now.getMonth();
    }
    const days = Number(mode);
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - days);
    return d >= cutoff;
  }

  function getFiltered(){
    const q = els.search.value.trim().toLowerCase();
    const typeVal = els.type.value;
    const catVal = els.category.value;
    const dateVal = els.date.value;

    let list = loadAllTransactions().filter(t => {
      if(typeVal !== 'all' && t.type !== typeVal) return false;
      if(catVal !== 'all' && t.category !== catVal) return false;
      if(!withinDateFilter(t.date, dateVal)) return false;
      if(q){
        const hay = (t.category + ' ' + (t.note || '') + ' ' + (t.paymentMethod || t.source || '')).toLowerCase();
        if(!hay.includes(q)) return false;
      }
      return true;
    });

    const [sortKey, sortDir] = els.sort.value.split('-');
    list.sort((a, b) => {
      let av, bv;
      if(sortKey === 'date'){ av = new Date(a.date).getTime(); bv = new Date(b.date).getTime(); }
      else { av = Number(a.amount || 0); bv = Number(b.amount || 0); }
      return sortDir === 'asc' ? av - bv : bv - av;
    });

    return list;
  }

  function updateStats(){
    const all = loadAllTransactions();
    const totalIn = all.filter(t => t.type === 'income').reduce((s, t) => s + Number(t.amount || 0), 0);
    const totalOut = all.filter(t => t.type === 'expense').reduce((s, t) => s + Number(t.amount || 0), 0);
    document.getElementById('statCount').textContent = all.length;
    document.getElementById('statIn').textContent = formatINR(totalIn);
    document.getElementById('statOut').textContent = formatINR(totalOut);
    document.getElementById('statNet').textContent = formatINR(totalIn - totalOut);
  }

  function render(){
    const all = loadAllTransactions();
    const filtered = getFiltered();

    if(!filtered.length){
      els.body.innerHTML = '';
      els.empty.style.display = 'block';
    } else {
      els.empty.style.display = 'none';
      els.body.innerHTML = filtered.map(t => `
        <tr>
          <td class="mono" style="white-space:nowrap;">${t.date || '—'}</td>
          <td>
            <div class="cat-cell">
              <div class="cat-icon">${CATEGORY_ICONS[t.category] || (t.type === 'income' ? '💰' : '📦')}</div>
              <div>
                <div class="cat-name">${t.category || 'Uncategorised'}</div>
                ${t.note ? `<div class="note-sub">${t.note}</div>` : ''}
              </div>
            </div>
          </td>
          <td>${t.paymentMethod || t.source || '—'}</td>
          <td><span class="type-badge ${t.type}">${t.type}</span></td>
          <td class="amount-cell ${t.type === 'income' ? 'in' : 'out'}">${t.type === 'income' ? '+' : '−'}${formatINR(t.amount)}</td>
          <td>
            <div class="row-actions">
              <button class="icon-btn" title="Edit" onclick="editTx('${t.id}','${t.type}')">✎</button>
              <button class="icon-btn danger" title="Delete" onclick="openDeleteModal('${t.id}','${t.type}')">✕</button>
            </div>
          </td>
        </tr>
      `).join('');
    }

    els.resultCount.textContent = `Showing ${filtered.length} of ${all.length}`;
    document.querySelectorAll('thead th[data-sort]').forEach(th => th.classList.remove('active'));
    const [sortKey] = els.sort.value.split('-');
    const activeTh = document.querySelector(`thead th[data-sort="${sortKey}"]`);
    if(activeTh) activeTh.classList.add('active');
  }

  window.editTx = function(id, type){
    if(type === 'expense'){
      window.location.href = `expense.html?id=${id}`;
    } else {
      // Income entry page isn't built yet — flag it rather than sending to a dead link.
      alert('Editing income entries will be available once the Add/Edit Income page is built.');
    }
  };

  window.openDeleteModal = function(id, type){
    pendingDeleteId = id;
    pendingDeleteType = type;
    document.getElementById('deleteModal').classList.add('show');
  };
  document.getElementById('cancelDelete').addEventListener('click', () => {
    pendingDeleteId = null; pendingDeleteType = null;
    document.getElementById('deleteModal').classList.remove('show');
  });
  document.getElementById('confirmDelete').addEventListener('click', () => {
    if(pendingDeleteId && pendingDeleteType){
      if(pendingDeleteType === 'expense'){
        saveExpenses(loadExpenses().filter(e => e.id !== pendingDeleteId));
      } else {
        saveIncome(loadIncome().filter(i => i.id !== pendingDeleteId));
      }
      showToast('Transaction deleted');
      updateStats();
      populateCategoryFilter();
      render();
    }
    pendingDeleteId = null; pendingDeleteType = null;
    document.getElementById('deleteModal').classList.remove('show');
  });

  function showToast(text){
    const toast = document.getElementById('toast');
    document.getElementById('toastText').textContent = text;
    toast.classList.add('show');
    clearTimeout(showToast._t);
    showToast._t = setTimeout(() => toast.classList.remove('show'), 2400);
  }

  document.querySelectorAll('thead th[data-sort]').forEach(th => {
    th.addEventListener('click', () => {
      const key = th.dataset.sort;
      const [curKey, curDir] = els.sort.value.split('-');
      const newDir = (curKey === key && curDir === 'desc') ? 'asc' : 'desc';
      els.sort.value = `${key}-${newDir}`;
      render();
    });
  });

  [els.search, els.type, els.category, els.date, els.sort].forEach(el => {
    el.addEventListener('input', render);
    el.addEventListener('change', render);
  });

  document.getElementById('clearFilters').addEventListener('click', () => {
    els.search.value = '';
    els.type.value = 'all';
    els.category.value = 'all';
    els.date.value = 'all';
    els.sort.value = 'date-desc';
    render();
  });

  populateCategoryFilter();
  updateStats();
  render();
</script>

</body>
</html>
