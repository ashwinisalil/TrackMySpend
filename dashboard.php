<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Dashboard — TrackMySpend</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,400;0,9..144,500;0,9..144,600;0,9..144,700;1,9..144,500&family=IBM+Plex+Mono:wght@400;500;600&family=IBM+Plex+Sans:wght@400;500;600&display=swap" rel="stylesheet">
<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.4/chart.umd.min.js"></script>
<style>
  :root{
    --cover: #101c16;
    --cover-deep: #0a1410;
    --paper: #f4efe1;
    --paper-dim: #ece5d2;
    --ink: #1c2620;
    --ink-soft: #4b5850;
    --money: #3f7a5c;
    --money-deep: #2c5842;
    --stamp: #b1462f;
    --gold: #c9a15a;
    --line: rgba(28,38,32,0.14);
    --line-on-cover: rgba(244,239,225,0.16);
  }

  *{ margin:0; padding:0; box-sizing:border-box; }
  html,body{ min-height:100%; }
  body{
    background:var(--paper);
    color:var(--ink);
    font-family:'IBM Plex Sans', sans-serif;
    -webkit-font-smoothing:antialiased;
  }
  @media (prefers-reduced-motion: reduce){
    *{ animation-duration:0.001ms !important; animation-iteration-count:1 !important; transition-duration:0.001ms !important; }
  }
  a{ color:inherit; text-decoration:none; }
  .display{ font-family:'Fraunces', serif; }
  .mono{ font-family:'IBM Plex Mono', monospace; }
  ::selection{ background:var(--gold); color:var(--cover-deep); }

  .wrap{ max-width:1180px; margin:0 auto; padding:0 32px; }

  /* ---------------- NAV ---------------- */
  nav{
    background:var(--cover);
    border-bottom:1px solid var(--line-on-cover);
  }
  .nav-inner{
    display:flex; align-items:center; justify-content:space-between;
    padding:16px 32px;
    max-width:1180px; margin:0 auto;
  }
  .brand{
    display:flex; align-items:center; gap:11px;
    font-family:'Fraunces', serif; font-weight:600; font-size:1.1rem;
    color:var(--paper);
  }
  .brand .mark{
    width:32px; height:32px; border-radius:7px; overflow:hidden; flex:none;
    background:var(--paper);
    box-shadow:0 2px 0 var(--money-deep);
  }
  .brand .mark img{ width:100%; height:100%; object-fit:cover; display:block; }

  .nav-links{ display:flex; gap:20px; font-size:0.86rem; }
  .nav-links a{ position:relative; color:rgba(244,239,225,0.68); transition:color 0.2s; padding-bottom:4px; }
  .nav-links a::after{ content:''; position:absolute; left:0; right:0; bottom:0; height:2px; background:var(--gold); transform:scaleX(0); transform-origin:left; transition:transform 0.25s ease; }
  .nav-links a:hover{ color:var(--paper); }
  .nav-links a:hover::after{ transform:scaleX(1); }
  .nav-links a.active{ color:var(--paper); }
  .nav-links a.active::after{ transform:scaleX(1); }

  .nav-user{ display:flex; align-items:center; gap:10px; }
  .nav-avatar{
    width:32px; height:32px; border-radius:50%;
    background:var(--money); color:var(--paper);
    display:flex; align-items:center; justify-content:center;
    font-family:'IBM Plex Mono', monospace; font-weight:600; font-size:0.78rem;
    border:1.5px solid var(--gold);
  }
  .nav-signout{
    font-size:0.82rem; color:rgba(244,239,225,0.55); border:1px solid var(--line-on-cover);
    padding:7px 13px; border-radius:5px; transition:opacity 0.2s;
  }
  .nav-signout:hover{ opacity:0.7; }
  @media (max-width: 760px){ .nav-links{ display:none; } }

  /* ---------------- PAGE HEADER ---------------- */
  .page-head{
    padding:44px 0 0;
    display:flex; align-items:flex-end; justify-content:space-between; gap:20px; flex-wrap:wrap;
  }
  .kicker{
    font-family:'IBM Plex Mono', monospace; font-size:0.72rem; letter-spacing:0.1em; text-transform:uppercase;
    color:var(--money-deep); margin-bottom:8px;
  }
  .page-head h1{ font-family:'Fraunces', serif; font-weight:600; font-size:2rem; }
  .page-head p{ margin-top:8px; color:var(--ink-soft); font-size:0.96rem; }
  .period-select{
    display:flex; align-items:center; gap:10px;
    background:#fff; border:1px solid var(--line); border-radius:8px;
    padding:8px 14px; font-family:'IBM Plex Mono', monospace; font-size:0.8rem; color:var(--ink-soft);
  }
  .period-select select{
    border:none; background:none; font-family:'IBM Plex Mono', monospace; font-size:0.82rem;
    color:var(--ink); font-weight:600; outline:none; cursor:pointer;
  }

  /* ---------------- SUMMARY CARDS ---------------- */
  .stats-row{
    margin-top:28px; display:grid; grid-template-columns:repeat(4,1fr); gap:1px;
    background:var(--line); border:1px solid var(--line); border-radius:10px; overflow:hidden;
  }
  .stat{ background:#fff; padding:22px 24px; position:relative; }
  .stat .label{ font-family:'IBM Plex Mono', monospace; font-size:0.66rem; letter-spacing:0.06em; text-transform:uppercase; color:var(--ink-soft); }
  .stat .value{ font-family:'Fraunces', serif; font-weight:600; font-size:1.7rem; margin-top:6px; color:var(--ink); }
  .stat .sub{ font-size:0.78rem; color:var(--ink-soft); margin-top:4px; }
  .stat .sub.up{ color:var(--money-deep); }
  .stat .sub.down{ color:var(--stamp); }
  .stat.balance{ background:var(--cover); }
  .stat.balance .label{ color:rgba(244,239,225,0.55); }
  .stat.balance .value{ color:var(--paper); }
  .stat.balance .sub{ color:var(--gold); }
  .budget-bar{
    margin-top:10px; height:6px; border-radius:4px; background:var(--paper-dim); overflow:hidden;
  }
  .budget-bar .fill{ height:100%; background:var(--money); border-radius:4px; }
  .budget-bar .fill.warn{ background:var(--gold); }
  .budget-bar .fill.over{ background:var(--stamp); }

  /* ---------------- CHART GRID ---------------- */
  .chart-grid{
    margin-top:28px; display:grid; grid-template-columns: 1.3fr 1fr; gap:24px; align-items:start;
  }
  .panel{
    background:#fff; border:1px solid var(--line); border-radius:10px; padding:26px 28px;
  }
  .panel + .panel{ margin-top:24px; }
  .panel-head{ display:flex; align-items:flex-start; justify-content:space-between; margin-bottom:20px; gap:12px; }
  .panel-head h3{ font-family:'Fraunces', serif; font-weight:600; font-size:1.14rem; }
  .panel-head .hint{ font-family:'IBM Plex Mono', monospace; font-size:0.68rem; color:var(--ink-soft); white-space:nowrap; }
  .chart-box{ position:relative; height:260px; }
  .chart-box.tall{ height:300px; }
  .legend-row{
    display:flex; flex-wrap:wrap; gap:10px 18px; margin-top:16px; padding-top:16px; border-top:1px dashed var(--line);
  }
  .legend-item{ display:flex; align-items:center; gap:7px; font-size:0.8rem; color:var(--ink-soft); }
  .legend-dot{ width:9px; height:9px; border-radius:50%; flex:none; }

  /* ---------------- QUICK LINKS ---------------- */
  .quick-grid{
    margin:24px 0 80px; display:grid; grid-template-columns: 1.3fr 1fr; gap:24px; align-items:start;
  }
  .list-row{
    display:flex; align-items:center; justify-content:space-between; gap:14px;
    padding:13px 0; border-bottom:1px solid var(--line);
  }
  .list-row:last-child{ border-bottom:none; }
  .txn-icon{
    width:36px; height:36px; border-radius:8px; flex:none;
    display:flex; align-items:center; justify-content:center;
    font-family:'IBM Plex Mono', monospace; font-size:0.78rem; font-weight:600;
    background:var(--paper-dim); color:var(--money-deep);
  }
  .txn-info{ flex:1; min-width:0; }
  .txn-info .t-name{ font-size:0.9rem; font-weight:600; }
  .txn-info .t-sub{ font-size:0.76rem; color:var(--ink-soft); margin-top:2px; }
  .txn-amt{ font-family:'IBM Plex Mono', monospace; font-size:0.9rem; font-weight:600; white-space:nowrap; }
  .txn-amt.out{ color:var(--stamp); }
  .txn-amt.in{ color:var(--money-deep); }

  .goal-row{ padding:14px 0; border-bottom:1px solid var(--line); }
  .goal-row:last-child{ border-bottom:none; }
  .goal-top{ display:flex; align-items:center; justify-content:space-between; margin-bottom:8px; }
  .goal-top .g-name{ font-size:0.9rem; font-weight:600; }
  .goal-top .g-pct{ font-family:'IBM Plex Mono', monospace; font-size:0.76rem; color:var(--money-deep); font-weight:600; }
  .goal-bar{ height:7px; border-radius:5px; background:var(--paper-dim); overflow:hidden; }
  .goal-bar .fill{ height:100%; background:var(--money); border-radius:5px; }
  .goal-sub{ margin-top:6px; font-size:0.76rem; color:var(--ink-soft); }

  .panel-link{
    font-family:'IBM Plex Mono', monospace; font-size:0.72rem; letter-spacing:0.03em;
    color:var(--money-deep); font-weight:600; border-bottom:1px solid var(--money-deep);
  }

  .tag-status{
    font-family:'IBM Plex Mono', monospace; font-size:0.64rem; letter-spacing:0.04em; text-transform:uppercase;
    padding:4px 9px; border-radius:4px; font-weight:600;
  }
  .tag-status.ok{ background:rgba(63,122,92,0.12); color:var(--money-deep); }
  .tag-status.warn{ background:rgba(201,161,90,0.18); color:#8a6a2e; }
  .tag-status.over{ background:rgba(177,70,47,0.1); color:var(--stamp); }

  @media (max-width: 900px){
    .chart-grid, .quick-grid{ grid-template-columns:1fr; }
    .stats-row{ grid-template-columns:1fr 1fr; }
  }
  @media (max-width: 520px){
    .stats-row{ grid-template-columns:1fr; }
  }
</style>
  <link rel="icon" type="image/png" href="data:image/png;base64,">
</head>
<body>

<nav>
  <div class="nav-inner">
    <a href="dashboard.html" class="brand">
      <span class="mark"><img src="data:image/png;base64," alt="TrackMySpend logo"></span>
      TrackMySpend
    </a>
    <div class="nav-links">
      <a href="dashboard.html" class="active">Dashboard</a>
      <a href="transactions.html">Transactions</a>
      <a href="budgets.html">Budgets</a>
      <a href="goals.html">Goals</a>
      <a href="reports.html">Reports</a>
      <a href="settings.html">Settings</a>
      <a href="profile.html">Profile</a>
    </div>
    <div class="nav-user">
      <span class="nav-avatar">RS</span>
      <a href="index.html" class="nav-signout">Sign out</a>
    </div>
  </div>
</nav>

<div class="wrap">

  <div class="page-head">
    <div>
      <div class="kicker">Overview</div>
      <h1>Dashboard</h1>
      <p>Where every rupee stands today, at a glance.</p>
    </div>
    <div class="period-select">
      <span>Showing</span>
      <select id="periodSelect">
        <option value="jul2026" selected>July 2026</option>
        <option value="jun2026">June 2026</option>
        <option value="may2026">May 2026</option>
        <option value="q2-2026">Last 3 months</option>
      </select>
    </div>
  </div>

  <!-- SUMMARY CARDS -->
  <div class="stats-row">
    <div class="stat balance">
      <div class="label">Total balance</div>
      <div class="value">₹1,84,230</div>
      <div class="sub">Across 3 linked accounts</div>
    </div>
    <div class="stat">
      <div class="label">Income this month</div>
      <div class="value">₹68,000</div>
      <div class="sub up">↑ 6% vs last month</div>
    </div>
    <div class="stat">
      <div class="label">Expense this month</div>
      <div class="value">₹41,860</div>
      <div class="sub down">↑ 12% vs last month</div>
    </div>
    <div class="stat">
      <div class="label">Budget status</div>
      <div class="value">73%</div>
      <div class="budget-bar"><div class="fill warn" style="width:73%"></div></div>
      <div class="sub">₹18,140 left of ₹60,000</div>
    </div>
  </div>

  <!-- CHARTS -->
  <div class="chart-grid">
    <div>
      <div class="panel">
        <div class="panel-head">
          <div>
            <h3>Income vs. expense</h3>
          </div>
          <span class="hint">Last 6 months</span>
        </div>
        <div class="chart-box">
          <canvas id="barChart"></canvas>
        </div>
      </div>

      <div class="panel">
        <div class="panel-head">
          <div>
            <h3>Spending &amp; saving trend</h3>
          </div>
          <span class="hint">Last 6 months</span>
        </div>
        <div class="chart-box">
          <canvas id="lineChart"></canvas>
        </div>
      </div>
    </div>

    <div class="panel">
      <div class="panel-head">
        <div>
          <h3>Where it went</h3>
        </div>
        <span class="hint">July 2026</span>
      </div>
      <div class="chart-box tall">
        <canvas id="pieChart"></canvas>
      </div>
      <div class="legend-row" id="pieLegend"></div>
    </div>
  </div>

  <!-- QUICK LINKS -->
  <div class="quick-grid">
    <div class="panel">
      <div class="panel-head">
        <div><h3>Recent transactions</h3></div>
        <a href="transactions.html" class="panel-link">View all →</a>
      </div>

      <div class="list-row">
        <div class="txn-icon">🛒</div>
        <div class="txn-info">
          <div class="t-name">Big Bazaar Groceries</div>
          <div class="t-sub">Food &amp; groceries · Today</div>
        </div>
        <div class="txn-amt out">−₹1,240</div>
      </div>
      <div class="list-row">
        <div class="txn-icon">🚕</div>
        <div class="txn-info">
          <div class="t-name">Ola ride to office</div>
          <div class="t-sub">Transport · Today</div>
        </div>
        <div class="txn-amt out">−₹186</div>
      </div>
      <div class="list-row">
        <div class="txn-icon">💼</div>
        <div class="txn-info">
          <div class="t-name">Salary — July</div>
          <div class="t-sub">Income · Yesterday</div>
        </div>
        <div class="txn-amt in">+₹68,000</div>
      </div>
      <div class="list-row">
        <div class="txn-icon">🎬</div>
        <div class="txn-info">
          <div class="t-name">Netflix subscription</div>
          <div class="t-sub">Entertainment · 2 days ago</div>
        </div>
        <div class="txn-amt out">−₹649</div>
      </div>
      <div class="list-row">
        <div class="txn-icon">🏠</div>
        <div class="txn-info">
          <div class="t-name">Rent — July</div>
          <div class="t-sub">Housing · 3 days ago</div>
        </div>
        <div class="txn-amt out">−₹18,000</div>
      </div>
    </div>

    <div class="panel">
      <div class="panel-head">
        <div><h3>Active goals</h3></div>
        <a href="goals.html" class="panel-link">Manage →</a>
      </div>

      <div class="goal-row">
        <div class="goal-top">
          <div class="g-name">Emergency fund</div>
          <div class="g-pct">62%</div>
        </div>
        <div class="goal-bar"><div class="fill" style="width:62%"></div></div>
        <div class="goal-sub">₹1,24,000 of ₹2,00,000</div>
      </div>
      <div class="goal-row">
        <div class="goal-top">
          <div class="g-name">New laptop</div>
          <div class="g-pct">38%</div>
        </div>
        <div class="goal-bar"><div class="fill" style="width:38%"></div></div>
        <div class="goal-sub">₹34,200 of ₹90,000</div>
      </div>
      <div class="goal-row">
        <div class="goal-top">
          <div class="g-name">Goa trip</div>
          <div class="g-pct">81%</div>
        </div>
        <div class="goal-bar"><div class="fill" style="width:81%"></div></div>
        <div class="goal-sub">₹32,400 of ₹40,000</div>
      </div>
    </div>
  </div>

</div>

<script>
  /* ---------------------------------------------------------------
     Sample aggregation payloads — shaped exactly like the JSON the
     backend's /api/dashboard/summary and /api/dashboard/charts
     endpoints would return (category sums + monthly totals).
     Swap this block for a fetch() call once the API is live:
       const res = await fetch('/api/dashboard/charts?period=2026-07');
       const data = await res.json();
  --------------------------------------------------------------- */
  const dashboardData = {
    months: ['Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul'],
    income:  [62000, 62000, 64500, 64500, 64000, 68000],
    expense: [38400, 41200, 36900, 44700, 37300, 41860],
    savingsTrend: [23600, 20800, 27600, 19800, 26700, 26140],
    categories: [
      { label: 'Housing',        value: 18000, color: '#2c5842' },
      { label: 'Food & groceries', value: 8420,  color: '#3f7a5c' },
      { label: 'Transport',      value: 4180,  color: '#6fa387' },
      { label: 'Entertainment',  value: 2860,  color: '#c9a15a' },
      { label: 'Utilities',      value: 3600,  color: '#b1462f' },
      { label: 'Other',          value: 4800,  color: '#8a8478' }
    ]
  };

  const paper = '#f4efe1';
  const ink = '#1c2620';
  const inkSoft = '#4b5850';
  const gridColor = 'rgba(28,38,32,0.08)';

  Chart.defaults.font.family = "'IBM Plex Sans', sans-serif";
  Chart.defaults.color = inkSoft;

  // Bar chart — monthly income vs expense
  new Chart(document.getElementById('barChart'), {
    type: 'bar',
    data: {
      labels: dashboardData.months,
      datasets: [
        { label: 'Income', data: dashboardData.income, backgroundColor: '#3f7a5c', borderRadius: 4, maxBarThickness: 26 },
        { label: 'Expense', data: dashboardData.expense, backgroundColor: '#c9a15a', borderRadius: 4, maxBarThickness: 26 }
      ]
    },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { position: 'top', align: 'end', labels: { boxWidth: 10, boxHeight: 10, usePointStyle: true, pointStyle: 'circle' } } },
      scales: {
        y: { grid: { color: gridColor }, ticks: { callback: (v) => '₹' + (v/1000) + 'k' } },
        x: { grid: { display: false } }
      }
    }
  });

  // Line chart — spending / saving trend
  new Chart(document.getElementById('lineChart'), {
    type: 'line',
    data: {
      labels: dashboardData.months,
      datasets: [
        {
          label: 'Net saved',
          data: dashboardData.savingsTrend,
          borderColor: '#2c5842',
          backgroundColor: 'rgba(63,122,92,0.12)',
          fill: true, tension: 0.35, pointRadius: 3, pointBackgroundColor: '#2c5842'
        },
        {
          label: 'Expense',
          data: dashboardData.expense,
          borderColor: '#b1462f',
          backgroundColor: 'transparent',
          borderDash: [4,4],
          fill: false, tension: 0.35, pointRadius: 2, pointBackgroundColor: '#b1462f'
        }
      ]
    },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { position: 'top', align: 'end', labels: { boxWidth: 10, boxHeight: 10, usePointStyle: true, pointStyle: 'circle' } } },
      scales: {
        y: { grid: { color: gridColor }, ticks: { callback: (v) => '₹' + (v/1000) + 'k' } },
        x: { grid: { display: false } }
      }
    }
  });

  // Pie chart — category-wise breakdown
  new Chart(document.getElementById('pieChart'), {
    type: 'doughnut',
    data: {
      labels: dashboardData.categories.map(c => c.label),
      datasets: [{
        data: dashboardData.categories.map(c => c.value),
        backgroundColor: dashboardData.categories.map(c => c.color),
        borderColor: paper, borderWidth: 3
      }]
    },
    options: {
      responsive: true, maintainAspectRatio: false, cutout: '62%',
      plugins: { legend: { display: false } }
    }
  });

  // Build the custom legend for the pie chart
  const legendEl = document.getElementById('pieLegend');
  const total = dashboardData.categories.reduce((s,c) => s + c.value, 0);
  dashboardData.categories.forEach(c => {
    const pct = Math.round((c.value / total) * 100);
    const item = document.createElement('div');
    item.className = 'legend-item';
    item.innerHTML = `<span class="legend-dot" style="background:${c.color}"></span>${c.label} · ${pct}%`;
    legendEl.appendChild(item);
  });
</script>

</body>
</html>
