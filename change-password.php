...
<title>Change Password — TrackMySpend</title>
...
<style>
  ...
  *{ margin:0; padding:0; box-sizing:border-box; }
  ...
  .btn-ghost{ border:1px solid var(--line-on-cover); color:var(--paper); background:transparent; }
  .btn-ghost:hover{ background:rgba(244,239,225,0.06); }

  /* ---------- PAGE HERO / KICKER (shared styling language) ---------- */
  section{ position:relative; }
  .section-pad{ padding:96px 0 110px; }
  .kicker{
  ...
  /* ---------- CHANGE PASSWORD CARD (styled like the passbook/donut cards) ---------- */
  .cp-wrap{
    max-width:1180px; margin:0 auto; padding:0 32px;
    display:flex; flex-direction:column; align-items:center;
  }
  .cp-head{ text-align:center; max-width:520px; margin-bottom:40px; }
  h1{
  ...
  .cp-head p{
    margin-top:14px; font-size:1.02rem; line-height:1.6; color:rgba(244,239,225,0.66);
  }
  ...
  .cp-card-inner{ padding:38px 34px 34px 60px; }

  .cp-card .label-row{
    display:flex; justify-content:space-between; align-items:flex-start;
    padding-bottom:16px; margin-bottom:26px; border-bottom:1px dashed var(--line);
  }
  .cp-card .label{ font-family:'IBM Plex Mono', monospace; font-size:0.68rem; letter-spacing:0.1em; text-transform:uppercase; color:var(--ink-soft); }
  .cp-card .acct{ font-family:'Fraunces', serif; font-weight:600; font-size:1.05rem; margin-top:4px; }
  .cp-card .stamp-icon{
    width:40px; height:40px; border-radius:50%;
    border:1.5px solid var(--gold); color:var(--money-deep);
    display:flex; align-items:center; justify-content:center;
  ...
  .field input{
    ...
    color:var(--ink); font-family:'IBM Plex Sans', sans-serif; font-size:0.95rem;
  ...
  .toggle-visibility{
    ...
    text-transform:uppercase; color:var(--ink-soft); padding:4px 6px;
  }
  .toggle-visibility:hover{ color:var(--money-deep); }

  .strength{ margin-top:9px; }
  .strength-bar{ height:5px; border-radius:4px; background:rgba(28,38,32,0.1); overflow:hidden; }
  .strength-fill{ height:100%; width:0%; border-radius:4px; background:var(--stamp); transition: width 0.25s ease, background 0.25s ease; }
  .strength-text{ margin-top:6px; font-family:'IBM Plex Mono', monospace; font-size:0.68rem; color:var(--ink-soft); }

  .hint{ font-size:0.78rem; color:var(--ink-soft); margin-top:6px; line-height:1.5; }
  .err-msg{ font-size:0.78rem; color:var(--stamp); margin-top:6px; line-height:1.5; display:none; }
  .err-msg.show{ display:block; }

  .cp-actions{ margin-top:30px; display:flex; flex-direction:column; gap:12px; }
  .cp-back{
    text-align:center; font-size:0.85rem; color:var(--ink-soft);
    padding-top:4px;
  }
  .cp-back a{ color:var(--money-deep); font-weight:600; }
  .cp-back a:hover{ text-decoration:underline; }

  .success-banner{
    display:none;
    margin-top:22px; padding:14px 16px; border-radius:6px;
  ...
  @media (max-width: 560px){
    ...
    .cp-card-inner{ padding:32px 22px 28px 46px; }
    .cp-card::before{ width:24px; }
  }
</style>
...
<section class="cover-bg section-pad">
  <div class="cp-wrap">
    <div class="cp-head">
      ...
      <p>Choose a new password to keep your TrackMySpend account secure. You'll need your current password to confirm the change.</p>
    </div>
    ...
    <div class="cp-card">
      <div class="cp-card-inner">
        <div class="label-row">
          <div>
            <div class="label">Security · Update</div>
            <div class="acct">Password change</div>
          </div>
        ...
        <form id="cpForm" novalidate>
          <div class="field">
            ...
            <div class="field-input">
              ...
              <button type="button" class="toggle-visibility" data-target="currentPassword">Show</button>
            </div>
            <div class="err-msg" id="err-current">Enter your current password.</div>
          </div>
          ...
          <div class="field">
            ...
            <div class="field-input">
              ...
              <button type="button" class="toggle-visibility" data-target="newPassword">Show</button>
            </div>
            <div class="strength">
              <div class="strength-bar"><div class="strength-fill" id="strengthFill"></div></div>
              <div class="strength-text" id="strengthText">Minimum 8 characters</div>
            </div>
            <div class="err-msg" id="err-new">Password must be at least 8 characters and different from your current password.</div>
          </div>
          ...
          <div class="success-banner" id="successBanner">Password updated successfully.</div>
        </form>

        <div class="cp-back">Forgot your current password? <a href="login.html">Reset it instead</a></div>
      </div>
    </div>
  </div>
</section>
...
<script>
  ...
  newPassword.addEventListener('input', () => {
    ...
    const pct = pwd.length === 0 ? 0 : Math.min(100, (score / 5) * 100);
    strengthFill.style.width = pct + '%';

    let label = 'Minimum 8 characters';
    let color = 'var(--stamp)';
    if(pwd.length > 0){
      if(score <= 1){ label = 'Weak'; color = 'var(--stamp)'; }
      else if(score <= 3){ label = 'Okay'; color = 'var(--gold)'; }
      else { label = 'Strong'; color = 'var(--money)'; }
    }
  ...
  form.addEventListener('submit', (e) => {
    ...
    if(currentPassword.value.trim().length === 0){
      showError('err-current', true);
      currentPassword.classList.add('error');
      valid = false;
    } else {
      showError('err-current', false);
      currentPassword.classList.remove('error');
    }

    const newVal = newPassword.value;
    if(newVal.length < 8 || (currentPassword.value && newVal === currentPassword.value)){
      showError('err-new', true);
      newPassword.classList.add('error');
      valid = false;
    } else {
      showError('err-new', false);
      newPassword.classList.remove('error');
    }

    if(confirmPassword.value !== newVal || confirmPassword.value.trim().length === 0){
      showError('err-confirm', true);
      confirmPassword.classList.add('error');
      valid = false;
    } else {
      showError('err-confirm', false);
      confirmPassword.classList.remove('error');
    }

    if(valid){
      // Placeholder for API call to update the password
