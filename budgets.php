<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Budget Management — TrackMySpend</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,400;0,9..144,500;0,9..144,600;0,9..144,700;1,9..144,500&family=IBM+Plex+Mono:wght@400;500;600&family=IBM+Plex+Sans:wght@400;500;600&display=swap" rel="stylesheet">
<style>
  :root{
    --cover: #101c16;
    --cover-deep: #0a1410;
    --paper: #f4efe1;
    --paper-dim: #ece5d2;
    --ink: #1c2620;
    --ink-soft: #4b5850;
    --money: #3f7a5c;
    --money-deep: #2c5842;
    --stamp: #b1462f;
    --gold: #c9a15a;
    --line: rgba(28,38,32,0.14);
    --line-on-cover: rgba(244,239,225,0.16);
  }
  *{ margin:0; padding:0; box-sizing:border-box; }
  html,body{ min-height:100%; }
  body{
    background:var(--paper);
    color:var(--ink);
    font-family:'IBM Plex Sans', sans-serif;
    -webkit-font-smoothing:antialiased;
  }
  @media (prefers-reduced-motion: reduce){
    *{ animation-duration:0.001ms !important; animation-iteration-count:1 !important; transition-duration:0.001ms !important; }
  }
  a{ color:inherit; text-decoration:none; }
  .display{ font-family:'Fraunces', serif; }
  .mono{ font-family:'IBM Plex Mono', monospace; }
  ::selection{ background:var(--gold); color:var(--cover-deep); }
  .wrap{ max-width:1180px; margin:0 auto; padding:0 32px; }
  button{ font-family:inherit; cursor:pointer; }

  /* ---------------- NAV ---------------- */
  nav{ background:var(--cover); border-bottom:1px solid var(--line-on-cover); position:sticky; top:0; z-index:60; }
  .nav-inner{ display:flex; align-items:center; justify-content:space-between; padding:16px 32px; max-width:1180px; margin:0 auto; }
  .brand{ display:flex; align-items:center; gap:11px; font-family:'Fraunces', serif; font-weight:600; font-size:1.1rem; color:var(--paper); }
  .brand .mark{ width:32px; height:32px; border-radius:7px; overflow:hidden; flex:none; background:var(--paper); box-shadow:0 2px 0 var(--money-deep); }
  .brand .mark img{ width:100%; height:100%; object-fit:cover; display:block; }
  .nav-links{ display:flex; gap:20px; font-size:0.86rem; }
  .nav-links a{ position:relative; color:rgba(244,239,225,0.68); transition:color 0.2s; padding-bottom:4px; }
  .nav-links a::after{ content:''; position:absolute; left:0; right:0; bottom:0; height:2px; background:var(--gold); transform:scaleX(0); transform-origin:left; transition:transform 0.25s ease; }
  .nav-links a:hover{ color:var(--paper); }
  .nav-links a:hover::after{ transform:scaleX(1); }
  .nav-links a.active{ color:var(--paper); }
  .nav-links a.active::after{ transform:scaleX(1); }
  .nav-user{ display:flex; align-items:center; gap:10px; }
  .nav-avatar{ width:32px; height:32px; border-radius:50%; background:var(--money); color:var(--paper); display:flex; align-items:center; justify-content:center; font-family:'IBM Plex Mono', monospace; font-weight:600; font-size:0.78rem; border:1.5px solid var(--gold); }
  .nav-signout{ font-size:0.82rem; color:rgba(244,239,225,0.55); border:1px solid var(--line-on-cover); padding:7px 13px; border-radius:5px; transition:opacity 0.2s; }
  .nav-signout:hover{ opacity:0.7; }
  @media (max-width: 860px){ .nav-links{ display:none; } }


  /* ---------------- PAGE HEAD ---------------- */
  .page-head{ padding:44px 0 0; display:flex; align-items:flex-end; justify-content:space-between; gap:20px; flex-wrap:wrap; }
  .kicker{ font-family:'IBM Plex Mono', monospace; font-size:0.72rem; letter-spacing:0.1em; text-transform:uppercase; color:var(--money-deep); margin-bottom:8px; }
  .page-head h1{ font-family:'Fraunces', serif; font-weight:600; font-size:2rem; }
  .page-head p{ margin-top:8px; color:var(--ink-soft); font-size:0.96rem; max-width:520px; }
  .add-btn{
    flex:none; padding:12px 22px; border-radius:6px; border:none; background:var(--money-deep); color:var(--paper);
    font-weight:600; font-size:0.88rem; box-shadow:0 3px 0 #1e3f2f; transition:transform 0.15s ease, box-shadow 0.15s ease;
    display:flex; align-items:center; gap:8px;
  }
  .add-btn:hover{ transform:translateY(-1px); box-shadow:0 4px 0 #1e3f2f; }
  .add-btn:active{ transform:translateY(1px); box-shadow:0 1px 0 #1e3f2f; }

  /* ---------------- SUBTABS ---------------- */
  .subtabs{ margin-top:26px; display:flex; gap:6px; background:#fff; border:1px solid var(--line); border-radius:8px; padding:5px; width:fit-content; }
  .subtabs a{ padding:9px 18px; border-radius:5px; font-size:0.86rem; font-weight:600; color:var(--ink-soft); transition:background 0.2s, color 0.2s; }
  .subtabs a.active{ background:var(--money-deep); color:var(--paper); }
  .subtabs a:not(.active):hover{ background:var(--paper-dim); color:var(--ink); }

  /* ---------------- STATS ---------------- */
  .stats-row{ margin-top:24px; display:grid; grid-template-columns:repeat(4,1fr); gap:1px; background:var(--line); border:1px solid var(--line); border-radius:10px; overflow:hidden; }
  .stat{ background:#fff; padding:20px 22px; transition:background 0.2s; }
  .stat:hover{ background:var(--paper-dim); }
  .stat .label{ font-family:'IBM Plex Mono', monospace; font-size:0.66rem; letter-spacing:0.06em; text-transform:uppercase; color:var(--ink-soft); }
  .stat .value{ font-family:'Fraunces', serif; font-weight:600; font-size:1.55rem; margin-top:6px; color:var(--ink); }
  .stat .sub{ font-size:0.76rem; color:var(--ink-soft); margin-top:4px; }
  .stat .sub.up{ color:var(--money-deep); }
  .stat .sub.warn{ color:var(--stamp); }

  /* ---------------- MAIN GRID ---------------- */
  .main-grid{ margin:32px 0 80px; display:grid; grid-template-columns:1.5fr 1fr; gap:24px; align-items:start; }
  .panel{ background:#fff; border:1px solid var(--line); border-radius:10px; padding:26px 28px; }
  .panel + .panel{ margin-top:24px; }
  .panel-head{ display:flex; align-items:center; justify-content:space-between; margin-bottom:20px; gap:12px; flex-wrap:wrap; }
  .panel-head h3{ font-family:'Fraunces', serif; font-weight:600; font-size:1.14rem; }
  .panel-head .hint{ font-family:'IBM Plex Mono', monospace; font-size:0.7rem; color:var(--ink-soft); }
  .empty-msg{ color:var(--ink-soft); font-size:0.86rem; padding:18px 0; text-align:center; }

  /* ---------------- BUDGET ROWS ---------------- */
  .budget-row{ padding:16px 0; border-bottom:1px solid var(--line); transition:background 0.15s ease; border-radius:6px; }
  .budget-row:last-child{ border-bottom:none; }
  .budget-row:hover{ background:var(--paper-dim); margin:0 -14px; padding:16px 14px; }
  .budget-top{ display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:10px; }
  .budget-name{ display:flex; align-items:center; gap:10px; font-weight:600; font-size:0.95rem; }
  .budget-icon{ width:34px; height:34px; border-radius:8px; background:var(--paper-dim); display:flex; align-items:center; justify-content:center; font-size:1.05rem; flex:none; }
  .budget-figures{ text-align:right; font-family:'IBM Plex Mono', monospace; font-size:0.82rem; color:var(--ink-soft); }
  .budget-figures b{ color:var(--ink); font-size:0.9rem; }
  .budget-actions{ display:flex; gap:6px; opacity:0; transition:opacity 0.15s; }
  .budget-row:hover .budget-actions{ opacity:1; }
  .icon-btn{ width:26px; height:26px; border-radius:5px; border:1px solid var(--line); background:#fff; display:flex; align-items:center; justify-content:center; font-size:0.78rem; color:var(--ink-soft); }
  .icon-btn:hover{ background:var(--paper-dim); color:var(--ink); }
  .bar-track{ height:9px; border-radius:5px; background:var(--paper-dim); overflow:hidden; }
  .bar-fill{ height:100%; border-radius:5px; background:var(--money); transition:width 0.4s ease; }
  .bar-fill.warn{ background:var(--gold); }
  .bar-fill.over{ background:var(--stamp); }
  .budget-meta{ display:flex; justify-content:space-between; margin-top:6px; font-size:0.72rem; color:var(--ink-soft); font-family:'IBM Plex Mono', monospace; }
  .pct-tag{ font-weight:600; }
  .pct-tag.warn{ color:#8a6a1f; }
  .pct-tag.over{ color:var(--stamp); }

  /* ---------------- TABLE ---------------- */
  table{ width:100%; border-collapse:collapse; font-size:0.86rem; }
  thead th{
    text-align:left; font-family:'IBM Plex Mono', monospace; font-size:0.66rem; letter-spacing:0.06em; text-transform:uppercase;
    color:var(--ink-soft); padding:0 10px 10px; border-bottom:1px solid var(--line); cursor:pointer; user-select:none;
    transition:color 0.15s;
  }
  thead th:hover{ color:var(--money-deep); }
  thead th .arrow{ font-size:0.6rem; margin-left:3px; opacity:0.5; }
  tbody td{ padding:12px 10px; border-bottom:1px solid var(--line); vertical-align:middle; }
  tbody tr{ transition:background 0.15s ease; }
  tbody tr:hover{ background:var(--paper-dim); }
  tbody tr:last-child td{ border-bottom:none; }
  .tx-desc{ font-weight:600; }
  .tx-date{ color:var(--ink-soft); font-family:'IBM Plex Mono', monospace; font-size:0.78rem; }
  .badge{ display:inline-block; padding:4px 10px; border-radius:20px; font-size:0.72rem; font-weight:600; background:var(--paper-dim); color:var(--ink-soft); }
  .amt{ font-family:'IBM Plex Mono', monospace; font-weight:600; text-align:right; }
  .amt.neg{ color:var(--stamp); }
  .row-del{ opacity:0; transition:opacity 0.15s; color:var(--ink-soft); font-size:0.85rem; }
  tbody tr:hover .row-del{ opacity:1; }
  .row-del:hover{ color:var(--stamp); }

  /* ---------------- FORM ---------------- */
  .field{ display:flex; flex-direction:column; gap:8px; margin-bottom:16px; }
  .field:last-of-type{ margin-bottom:0; }
  .field-row{ display:grid; grid-template-columns:1fr 1fr; gap:14px; }
  .field label{ font-family:'IBM Plex Mono', monospace; font-size:0.7rem; letter-spacing:0.06em; text-transform:uppercase; color:var(--ink-soft); }
  input[type="text"], input[type="number"], input[type="date"], select{
    width:100%; padding:11px 13px; border-radius:6px; border:1px solid var(--line); background:#fff;
    font-family:'IBM Plex Sans', sans-serif; font-size:0.9rem; color:var(--ink); transition:border-color 0.15s ease, box-shadow 0.15s ease;
  }
  input:focus, select:focus{ outline:none; border-color:var(--money); box-shadow:0 0 0 3px rgba(63,122,92,0.14); }
  .icon-pick{ display:flex; gap:8px; flex-wrap:wrap; }
  .icon-opt{ width:38px; height:38px; border-radius:7px; border:1.5px solid var(--line); background:#fff; display:flex; align-items:center; justify-content:center; font-size:1.05rem; transition:all 0.15s; }
  .icon-opt.selected{ border-color:var(--money); background:rgba(63,122,92,0.1); }
  .btn{ padding:11px 20px; border-radius:6px; font-weight:600; font-size:0.88rem; border:none; cursor:pointer; transition:transform 0.15s ease, box-shadow 0.15s ease; }
  .btn-primary{ background:var(--money-deep); color:var(--paper); box-shadow:0 3px 0 #1e3f2f; }
  .btn-primary:hover{ transform:translateY(-1px); box-shadow:0 4px 0 #1e3f2f; }
  .btn-primary:active{ transform:translateY(1px); box-shadow:0 1px 0 #1e3f2f; }
  .btn-ghost{ background:none; border:1px solid var(--line); color:var(--ink-soft); }
  .btn-ghost:hover{ background:var(--paper-dim); }
  .btn-block{ width:100%; }

  /* ---------------- ALERTS ---------------- */
  .alert-item{ display:flex; gap:12px; padding:13px 0; border-bottom:1px dashed rgba(177,70,47,0.25); }
  .alert-item:last-child{ border-bottom:none; }
  .alert-dot{ width:8px; height:8px; border-radius:50%; background:var(--stamp); margin-top:6px; flex:none; }
  .alert-text b{ display:block; font-size:0.88rem; }
  .alert-text span{ font-size:0.78rem; color:var(--ink-soft); }
  .alert-ok{ display:flex; align-items:center; gap:10px; color:var(--money-deep); font-size:0.88rem; padding:6px 0; }

  /* ---------------- MINI CHART ---------------- */
  .chart-row{ display:flex; align-items:flex-end; gap:10px; height:120px; margin-top:6px; }
  .chart-col{ flex:1; display:flex; flex-direction:column; align-items:center; gap:8px; height:100%; justify-content:flex-end; }
  .chart-bar{ width:100%; border-radius:5px 5px 0 0; background:linear-gradient(180deg, var(--money), var(--money-deep)); transition:height 0.4s ease; position:relative; }
  .chart-bar:hover::after{
    content:attr(data-val); position:absolute; top:-24px; left:50%; transform:translateX(-50%);
    background:var(--cover); color:var(--paper); font-family:'IBM Plex Mono', monospace; font-size:0.68rem; padding:3px 7px; border-radius:4px; white-space:nowrap;
  }
  .chart-label{ font-family:'IBM Plex Mono', monospace; font-size:0.66rem; color:var(--ink-soft); }

  /* ---------------- MODAL ---------------- */
  .modal-backdrop{ position:fixed; inset:0; background:rgba(16,28,22,0.55); backdrop-filter:blur(2px); display:none; align-items:center; justify-content:center; z-index:100; padding:20px; }
  .modal-backdrop.show{ display:flex; }
  .modal{ background:#fff; border-radius:12px; padding:30px 32px; width:100%; max-width:440px; box-shadow:0 20px 60px rgba(0,0,0,0.3); }
  .modal h3{ font-family:'Fraunces', serif; font-weight:600; font-size:1.2rem; margin-bottom:18px; }
  .modal-actions{ display:flex; justify-content:flex-end; gap:10px; margin-top:20px; }

  .toast{
    position:fixed; bottom:26px; right:26px; background:var(--cover); color:var(--paper); padding:13px 20px; border-radius:8px;
    font-size:0.86rem; box-shadow:0 8px 24px rgba(0,0,0,0.25); display:flex; align-items:center; gap:10px;
    transform:translateY(20px); opacity:0; pointer-events:none; transition:all 0.3s ease; z-index:200;
  }
  .toast.show{ transform:translateY(0); opacity:1; }
  .toast .dot{ width:7px; height:7px; border-radius:50%; background:var(--gold); }

  @media (max-width: 900px){
    .main-grid{ grid-template-columns:1fr; }
    .stats-row{ grid-template-columns:1fr 1fr; }
    .field-row{ grid-template-columns:1fr; }
  }
</style>
  <link rel="icon" type="image/png" href="data:image/png;base64,">
</head>
<body>

<nav>
  <div class="nav-inner">
    <a href="dashboard.html" class="brand">
      <span class="mark"><img src="data:image/png;base64," alt="TrackMySpend logo"></span>
      TrackMySpend
    </a>
    <div class="nav-links">
      <a href="dashboard.html">Dashboard</a>
      <a href="transactions.html">Transactions</a>
      <a href="budgets.html" class="active">Budgets</a>
      <a href="goals.html">Goals</a>
      <a href="reports.html">Reports</a>
      <a href="settings.html">Settings</a>
      <a href="profile.html">Profile</a>
    </div>
    <div class="nav-user">
      <span class="nav-avatar">RS</span>
      <a href="index.html" class="nav-signout">Sign out</a>
    </div>
  </div>
</nav>

<div class="wrap">

  <div class="page-head">
    <div>
      <div class="kicker">Money Management</div>
      <h1>Budget Management</h1>
      <p>Set monthly limits for every category, watch spending against them in real time, and catch overspending before it happens.</p>
    </div>
    <button class="add-btn" id="openAddBudget">+ New budget</button>
  </div>

  <div class="subtabs">
    <a href="budgets.html" class="active">Budget Overview</a>
    <a href="goals.html">Savings Goals</a>
    <a href="categories.html">Manage Categories</a>
  </div>

  <div class="stats-row" id="statsRow"></div>

  <div class="main-grid">

    <!-- LEFT COLUMN -->
    <div>
      <div class="panel">
        <div class="panel-head">
          <h3>Category budgets</h3>
          <span class="hint" id="periodHint">July 2026</span>
        </div>
        <div id="budgetList"></div>
      </div>

      <div class="panel">
        <div class="panel-head">
          <h3>Recent transactions</h3>
          <span class="hint">Click a column to sort</span>
        </div>
        <table>
          <thead>
            <tr>
              <th data-key="date">Date <span class="arrow">↕</span></th>
              <th data-key="desc">Description <span class="arrow">↕</span></th>
              <th data-key="category">Category <span class="arrow">↕</span></th>
              <th data-key="amount" style="text-align:right;">Amount <span class="arrow">↕</span></th>
              <th></th>
            </tr>
          </thead>
          <tbody id="txBody"></tbody>
        </table>
      </div>
    </div>

    <!-- RIGHT COLUMN -->
    <div>
      <div class="panel">
        <div class="panel-head">
          <h3>Spending — last 6 months</h3>
        </div>
        <div class="chart-row" id="chartRow"></div>
      </div>

      <div class="panel">
        <div class="panel-head">
          <h3>Alerts</h3>
        </div>
        <div id="alertsPanel"></div>
      </div>
    </div>

  </div>
</div>

<!-- ADD / EDIT BUDGET MODAL -->
<div class="modal-backdrop" id="budgetModal">
  <div class="modal">
    <h3 id="modalTitle">New budget</h3>
    <form id="budgetForm">
      <input type="hidden" id="budgetId">
      <div class="field">
        <label>Category name</label>
        <input type="text" id="bName" placeholder="e.g. Groceries" required>
      </div>
      <div class="field">
        <label>Icon</label>
        <div class="icon-pick" id="iconPick">
          <button type="button" class="icon-opt selected" data-icon="🛒">🛒</button>
          <button type="button" class="icon-opt" data-icon="🏠">🏠</button>
          <button type="button" class="icon-opt" data-icon="🚌">🚌</button>
          <button type="button" class="icon-opt" data-icon="🍔">🍔</button>
          <button type="button" class="icon-opt" data-icon="🎬">🎬</button>
          <button type="button" class="icon-opt" data-icon="💡">💡</button>
          <button type="button" class="icon-opt" data-icon="🛍️">🛍️</button>
          <button type="button" class="icon-opt" data-icon="💊">💊</button>
        </div>
      </div>
      <div class="field-row">
        <div class="field">
          <label>Monthly limit (₹)</label>
          <input type="number" id="bLimit" min="0" step="1" required>
        </div>
        <div class="field">
          <label>Already spent (₹)</label>
          <input type="number" id="bSpent" min="0" step="1" value="0">
        </div>
      </div>
      <div class="modal-actions">
        <button type="button" class="btn btn-ghost" id="cancelBudget">Cancel</button>
        <button type="submit" class="btn btn-primary">Save budget</button>
      </div>
    </form>
  </div>
</div>

<div class="toast" id="toast"><span class="dot"></span><span id="toastMsg">Saved</span></div>

<script>
/* ============ STATE ============ */
const fmt = n => '₹' + Number(n).toLocaleString('en-IN', {maximumFractionDigits:0});

const seedBudgets = [
  { id:'b1', name:'Groceries',    icon:'🛒', limit:8000, spent:6120 },
  { id:'b2', name:'Rent',         icon:'🏠', limit:15000, spent:15000 },
  { id:'b3', name:'Transport',    icon:'🚌', limit:2500, spent:1780 },
  { id:'b4', name:'Dining Out',   icon:'🍔', limit:3000, spent:3260 },
  { id:'b5', name:'Entertainment',icon:'🎬', limit:2000, spent:940 },
  { id:'b6', name:'Utilities',    icon:'💡', limit:2800, spent:2340 },
];

const seedTx = [
  { id:'t1', date:'2026-07-18', desc:'BigBasket order',        category:'Groceries',     amount:1240 },
  { id:'t2', date:'2026-07-17', desc:'Zomato — dinner',        category:'Dining Out',    amount:560 },
  { id:'t3', date:'2026-07-16', desc:'Ola rides (week)',       category:'Transport',     amount:420 },
  { id:'t4', date:'2026-07-15', desc:'Electricity bill',       category:'Utilities',     amount:1380 },
  { id:'t5', date:'2026-07-14', desc:'PVR — movie tickets',    category:'Entertainment', amount:520 },
  { id:'t6', date:'2026-07-12', desc:'Monthly rent',           category:'Rent',          amount:15000 },
  { id:'t7', date:'2026-07-10', desc:'Reliance Fresh',         category:'Groceries',     amount:980 },
  { id:'t8', date:'2026-07-08', desc:'Swiggy — lunch',         category:'Dining Out',    amount:340 },
];

const seedChart = [
  { m:'Feb', v:24500 }, { m:'Mar', v:27800 }, { m:'Apr', v:22100 },
  { m:'May', v:29600 }, { m:'Jun', v:26300 }, { m:'Jul', v:29440 },
];

function load(key, fallback){
  const raw = localStorage.getItem(key);
  if(raw){ try { return JSON.parse(raw); } catch(e){ return fallback; } }
  localStorage.setItem(key, JSON.stringify(fallback));
  return fallback;
}
function save(key, val){ localStorage.setItem(key, JSON.stringify(val)); }

let budgets = load('tms_budgets', seedBudgets);
let txs = load('tms_transactions', seedTx);
let sortState = { key:'date', dir:-1 };

/* ============ RENDER: STATS ============ */
function renderStats(){
  const totalLimit = budgets.reduce((s,b)=>s+b.limit,0);
  const totalSpent = budgets.reduce((s,b)=>s+b.spent,0);
  const remaining = totalLimit - totalSpent;
  const overCount = budgets.filter(b=>b.spent > b.limit).length;

  const stats = [
    { label:'Total budgeted', value:fmt(totalLimit), sub:budgets.length+' categories' },
    { label:'Total spent', value:fmt(totalSpent), sub:(totalSpent<=totalLimit?'On track this month':'Over allocated total'), cls:(totalSpent<=totalLimit?'up':'warn') },
    { label:'Remaining', value:fmt(Math.max(remaining,0)), sub:(remaining>=0?'Left to spend':'Deficit'), cls:(remaining>=0?'up':'warn') },
    { label:'Over budget', value:overCount, sub:(overCount ? 'Categories need attention' : 'Everything under control'), cls:(overCount?'warn':'up') },
  ];
  document.getElementById('statsRow').innerHTML = stats.map(s=>`
    <div class="stat">
      <div class="label">${s.label}</div>
      <div class="value">${s.value}</div>
      <div class="sub ${s.cls||''}">${s.sub}</div>
    </div>
  `).join('');
}

/* ============ RENDER: BUDGET LIST ============ */
function renderBudgets(){
  const list = document.getElementById('budgetList');
  if(!budgets.length){ list.innerHTML = '<div class="empty-msg">No budgets yet. Click "New budget" to create one.</div>'; return; }

  list.innerHTML = budgets.map(b=>{
    const pct = Math.min(Math.round((b.spent / b.limit) * 100), 999);
    const barClass = pct >= 100 ? 'over' : pct >= 80 ? 'warn' : '';
    const tagClass = pct >= 100 ? 'over' : pct >= 80 ? 'warn' : '';
    return `
    <div class="budget-row">
      <div class="budget-top">
        <div class="budget-name">
          <span class="budget-icon">${b.icon}</span>
          <span>${b.name}</span>
        </div>
        <div style="display:flex; align-items:center; gap:14px;">
          <div class="budget-figures"><b>${fmt(b.spent)}</b> / ${fmt(b.limit)}</div>
          <div class="budget-actions">
            <button class="icon-btn" onclick="openEditBudget('${b.id}')" title="Edit">✎</button>
            <button class="icon-btn" onclick="deleteBudget('${b.id}')" title="Delete">✕</button>
          </div>
        </div>
      </div>
      <div class="bar-track"><div class="bar-fill ${barClass}" style="width:${Math.min(pct,100)}%"></div></div>
      <div class="budget-meta">
        <span class="pct-tag ${tagClass}">${pct}% used</span>
        <span>${b.limit - b.spent >= 0 ? fmt(b.limit-b.spent)+' left' : fmt(b.spent-b.limit)+' over'}</span>
      </div>
    </div>`;
  }).join('');
}

/* ============ RENDER: TABLE ============ */
function renderTable(){
  const body = document.getElementById('txBody');
  let rows = [...txs];
  rows.sort((a,b)=>{
    let av=a[sortState.key], bv=b[sortState.key];
    if(sortState.key==='amount'){ av=Number(av); bv=Number(bv); }
    if(av<bv) return -1*sortState.dir;
    if(av>bv) return 1*sortState.dir;
    return 0;
  });
  if(!rows.length){ body.innerHTML = '<tr><td colspan="5" class="empty-msg">No transactions logged.</td></tr>'; return; }
  body.innerHTML = rows.map(t=>`
    <tr>
      <td class="tx-date">${t.date}</td>
      <td class="tx-desc">${t.desc}</td>
      <td><span class="badge">${t.category}</span></td>
      <td class="amt neg">− ${fmt(t.amount)}</td>
      <td style="text-align:right;"><span class="row-del" onclick="deleteTx('${t.id}')" title="Remove">✕</span></td>
    </tr>
  `).join('');
}

/* ============ RENDER: CHART ============ */
function renderChart(){
  const max = Math.max(...seedChart.map(c=>c.v));
  document.getElementById('chartRow').innerHTML = seedChart.map(c=>`
    <div class="chart-col">
      <div class="chart-bar" data-val="${fmt(c.v)}" style="height:${Math.round((c.v/max)*100)}%"></div>
      <div class="chart-label">${c.m}</div>
    </div>
  `).join('');
}

/* ============ RENDER: ALERTS ============ */
function renderAlerts(){
  const panel = document.getElementById('alertsPanel');
  const risky = budgets.filter(b => (b.spent / b.limit) >= 0.8);
  if(!risky.length){
    panel.innerHTML = `<div class="alert-ok"><span class="dot" style="background:var(--money);width:8px;height:8px;border-radius:50%;"></span> All categories are within a healthy range.</div>`;
    return;
  }
  panel.innerHTML = risky.map(b=>{
    const pct = Math.round((b.spent/b.limit)*100);
    const over = b.spent > b.limit;
    return `
    <div class="alert-item">
      <div class="alert-dot"></div>
      <div class="alert-text">
        <b>${b.name} ${over ? 'is over budget' : 'is nearly at its limit'}</b>
        <span>${pct}% of ${fmt(b.limit)} used (${fmt(b.spent)} spent)</span>
      </div>
    </div>`;
  }).join('');
}

function renderAll(){ renderStats(); renderBudgets(); renderTable(); renderChart(); renderAlerts(); }
renderAll();

/* ============ SORTING ============ */
document.querySelectorAll('thead th[data-key]').forEach(th=>{
  th.addEventListener('click', ()=>{
    const key = th.dataset.key;
    if(sortState.key === key){ sortState.dir *= -1; } else { sortState.key = key; sortState.dir = 1; }
    renderTable();
  });
});

/* ============ DELETE ============ */
function deleteTx(id){
  txs = txs.filter(t=>t.id!==id);
  save('tms_transactions', txs);
  renderTable();
  showToast('Transaction removed');
}
function deleteBudget(id){
  budgets = budgets.filter(b=>b.id!==id);
  save('tms_budgets', budgets);
  renderAll();
  showToast('Budget deleted');
}

/* ============ MODAL / ADD / EDIT ============ */
const modal = document.getElementById('budgetModal');
const form = document.getElementById('budgetForm');
let selectedIcon = '🛒';

document.getElementById('openAddBudget').addEventListener('click', ()=>{
  document.getElementById('modalTitle').textContent = 'New budget';
  form.reset();
  document.getElementById('budgetId').value = '';
  selectedIcon = '🛒';
  setIconSelection(selectedIcon);
  modal.classList.add('show');
});
document.getElementById('cancelBudget').addEventListener('click', ()=> modal.classList.remove('show'));
modal.addEventListener('click', e=>{ if(e.target === modal) modal.classList.remove('show'); });

document.querySelectorAll('.icon-opt').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    selectedIcon = btn.dataset.icon;
    setIconSelection(selectedIcon);
  });
});
function setIconSelection(icon){
  document.querySelectorAll('.icon-opt').forEach(b=>{
    b.classList.toggle('selected', b.dataset.icon === icon);
  });
}

function openEditBudget(id){
  const b = budgets.find(x=>x.id===id);
  if(!b) return;
  document.getElementById('modalTitle').textContent = 'Edit budget';
  document.getElementById('budgetId').value = b.id;
  document.getElementById('bName').value = b.name;
  document.getElementById('bLimit').value = b.limit;
  document.getElementById('bSpent').value = b.spent;
  selectedIcon = b.icon;
  setIconSelection(selectedIcon);
  modal.classList.add('show');
}

form.addEventListener('submit', e=>{
  e.preventDefault();
  const id = document.getElementById('budgetId').value;
  const name = document.getElementById('bName').value.trim();
  const limit = Number(document.getElementById('bLimit').value);
  const spent = Number(document.getElementById('bSpent').value) || 0;
  if(!name || limit <= 0) return;

  if(id){
    const b = budgets.find(x=>x.id===id);
    b.name = name; b.icon = selectedIcon; b.limit = limit; b.spent = spent;
    showToast('Budget updated');
  } else {
    budgets.push({ id:'b'+Date.now(), name, icon:selectedIcon, limit, spent });
    showToast('Budget created');
  }
  save('tms_budgets', budgets);
  modal.classList.remove('show');
  renderAll();
});

/* ============ TOAST ============ */
let toastTimer;
function showToast(msg){
  const toast = document.getElementById('toast');
  document.getElementById('toastMsg').textContent = msg;
  toast.classList.add('show');
  clearTimeout(toastTimer);
  toastTimer = setTimeout(()=> toast.classList.remove('show'), 2400);
}
</script>

</body>
</html>
